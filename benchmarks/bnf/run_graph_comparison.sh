#!/bin/sh
set -eu
if [ "$#" -lt 4 ] || [ "$#" -gt 9 ]; then
    echo 'usage: run_graph_comparison.sh RUNTIME PUBLIC_PROGRAM CANDIDATE_PROGRAM EVIDENCE_DIRECTORY [RULE_COUNTS] [TRIALS] [SECONDS_PER_RUN] [indexed|worklist] [linear|mapped]' >&2
    exit 2
fi
runtime=$(realpath "$1")
public=$(realpath "$2")
candidate=$(realpath "$3")
mkdir -p "$4"
evidence=$(realpath "$4")
counts=${5:-'25 50 100 200 1000'}
trials=${6:-3}
deadline=${7:-60}
selected=${8:-indexed}
selected_declarations=${9:-linear}
case $selected in indexed|worklist) :;; *) echo 'invalid candidate graph route' >&2; exit 2;; esac
case $selected_declarations in linear|mapped) :;; *) echo 'invalid candidate declaration route' >&2; exit 2;; esac
if [ "$selected_declarations" = mapped ] && [ "$selected" != worklist ]; then
    echo 'mapped declarations require the worklist candidate program' >&2
    exit 2
fi
for number in $counts "$trials" "$deadline"; do
    case $number in ''|*[!0-9]*|0) echo "invalid positive integer: $number" >&2; exit 2;; esac
done
test -x "$runtime" && test -f "$public" && test -f "$candidate"
test ! -e "$evidence/results.tsv"
root=$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd)
cd "$root"
${CC:-cc} -std=c11 -O2 -Wall -Wextra -Werror \
    benchmarks/bnf/generate_chain.c -o "$evidence/generate-chain"
sha256sum "$runtime" "$public" "$candidate" lib/lib_bnf.metta \
    langdef/bnf/*.metta benchmarks/bnf/generate_chain.c \
    benchmarks/bnf/run_graph_comparison.sh > "$evidence/inputs.sha256"
git rev-parse HEAD > "$evidence/base-commit.txt"
git diff --cached | sha256sum > "$evidence/staged-diff.sha256"
printf 'trial\trules\torder\tdeclarations\tgraph\tstatus\telapsed_s\tpeak_rss_kib\texit_code\n' > "$evidence/results.tsv"
trial=1
pair=0
while [ "$trial" -le "$trials" ]; do
    for count in $counts; do
        for order in forward reverse; do
            case $(((trial + pair) % 2)) in
                0) routes="$selected linear";;
                1) routes="linear $selected";;
            esac
            for route in $routes; do
                stem="$evidence/t${trial}-n${count}-${order}-${route}"
                program=$public
                declarations=linear
                if [ "$route" = "$selected" ]; then
                    program=$candidate
                    declarations=$selected_declarations
                fi
                "$evidence/generate-chain" "$count" validate "$declarations" "$order" "$route" > "$stem.metta"
                sha256sum "$stem.metta" >> "$evidence/workloads.sha256"
                code=0
                /usr/bin/time -f '%e\t%M\t%x' -o "$stem.time.tsv" \
                    timeout "$deadline" env CETTA_PETTA_SEARCH_MACHINE=1 CETTA_PETTA_MACHINE_STATS=1 \
                    "$runtime" --lang petta "$program" lib/lib_bnf.metta "$stem.metta" \
                    > "$stem.stdout" 2> "$stem.stderr" || code=$?
                status=failed
                if [ "$code" -eq 124 ]; then
                    status=timeout
                elif [ "$code" -eq 0 ] &&
                     ! rg -v '^PETTA_MACHINE_STATS ' "$stem.stderr" | rg -q . &&
                     [ "$(rg -F -x -c "(BnfBenchAccepted $count)" "$stem.stdout" || true)" = 1 ] &&
                     ! rg -q 'Error|BnfBench.*Refused|assertEqual|bench:' "$stem.stdout"; then
                    status=accepted
                fi
                printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
                    "$trial" "$count" "$order" "$declarations" "$route" "$status" \
                    "$(tail -n 1 "$stem.time.tsv")" | tee -a "$evidence/results.tsv"
            done
            pair=$((pair + 1))
        done
    done
    trial=$((trial + 1))
done
sha256sum -c "$evidence/inputs.sha256" > "$evidence/inputs-after.txt"
awk -F '\t' 'NR > 1 && $6 != "accepted" { bad = 1 } END { exit bad }' "$evidence/results.tsv"
