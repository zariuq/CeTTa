#!/usr/bin/env python3
"""Compare MeTTa-Interact realizations under the declared bag observer."""
...
def run(cetta: Path, source: Path, realization: str | None) -> str:
    command = [str(cetta), "--lang", "metta-interact"]
...
def main() -> int:
    ...
    if not sources:
        raise GateFailure("no MeTTa-Interact fixtures found")
    ...
    for source in sources:
        ...
        for realization in ("horn-reference", "compiled-worklist", None):
