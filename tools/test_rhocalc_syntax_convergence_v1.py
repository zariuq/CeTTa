...
CASES = (
    SyntaxCase("strict-nil", "0", "rho:nil", "accepted"),
    SyntaxCase(
        "strict-send",
        "@{0}!(0)",
...
def normalize_result(result: str, syntax: str, label: str) -> str:
    forms = parse_sexprs(result, source=f"{syntax}/{label}")
    if len(forms) != 1 or not tagged(forms[0], "result", 3):
        raise GateFailure(f"{syntax}/{label}: malformed semantic result")
    form = forms[0]
    assert isinstance(form, tuple)
    if not is_symbol(form[2], "nil"):
        raise GateFailure(f"{syntax}/{label}: parser left trailing input")
    document_label, process = node_parts(form[1], f"{syntax}/{label}/document")
    if document_label != "document":
        raise GateFailure(f"{syntax}/{label}: missing document node")
    return render(normalize_process(process, (), f"{syntax}/{label}"))
...
def oracle_rows(petta_root: Path) -> dict[tuple[str, str], dict[str, object]]:
    oracle = (
        ...
        / "rhocalc_syntax_convergence_oracle_v1.pl"
    )
    ...
    if completed.returncode != 0:
        raise GateFailure("PeTTa syntax oracle failed: " + completed.stderr.strip())
    lines = completed.stdout.splitlines()
    if (
        not lines
        or lines[0] != "rhocalc-syntax-convergence-oracle-v1"
        or lines[-1] != "end"
    ):
        raise GateFailure("PeTTa syntax oracle returned malformed framing")
    ...
    for line in lines[1:-1]:
        ...
        if fields[0] == "case" and len(fields) == 5 and current is None:
            ...
            if key in rows:
                raise GateFailure(f"duplicate PeTTa syntax row: {key}")
            current = {
                "decision": fields[3],
            ...
            raise GateFailure(f"malformed PeTTa syntax record: {line!r}")
    if current is not None:
        raise GateFailure("PeTTa syntax oracle returned an unterminated row")
    return rows
...
def require_exact_backend(
    ...
    for field in ("decision", "forest-digest", "results"):
        if observed.get(field) != oracle[field]:
            raise GateFailure(
                f"{syntax}/{label}: {backend} {field} differs from PeTTa"
            )
    ...
    for field, expected in (
        ...
        if observed.get(field) != expected:
            raise GateFailure(
                f"{syntax}/{label}: {backend} one-pass receipt changed"
            )
...
def row_text(syntax: str, label: str, row: dict[str, object]) -> str:
    results = row["results"]
    assert isinstance(results, tuple)
    return (
        f"{syntax}/{label}/{row['decision']}/{row['forest-digest']}:"
        + "|".join(results)
    )
...
def check(
    ...
    expected_keys = {
        (syntax, case.label) for case in CASES for syntax in ("rho", "mrho")
    }
    if set(expected) != expected_keys:
        raise GateFailure("PeTTa syntax oracle case inventory changed")
    ...
    rejected_pairs = 0
    records: list[str] = []
    with tempfile.TemporaryDirectory(prefix="gslt2parse-rho-syntaxes-") as raw:
        temp = Path(raw)
        pack_paths: dict[str, Path] = {}
        projection_paths: dict[str, Path] = {}
        starts: dict[str, str] = {}
        for syntax, plan in compile_plans().items():
            plan_path = temp / f"{syntax}.projection"
            plan_path.write_text(render(plan), encoding="utf-8")
            projection_paths[syntax] = plan_path
        for syntax, abi_label in (("rho", "rho"), ("mrho", "rho-mrho")):
            abi_case = ABI_CASES[abi_label]
            pack_path = temp / f"{syntax}.abi"
            pack_path.write_bytes(export_stream(petta_root, abi_case))
            pack_paths[syntax] = pack_path
            starts[syntax] = f"(pp-def {abi_case['start']})"

        for index, case in enumerate(CASES):
            normalized: dict[str, str] = {}
            projections: dict[str, dict[str, str]] = {}
            for syntax in ("rho", "mrho"):
                source = getattr(case, syntax)
                input_path = temp / f"input-{index}-{syntax}.utf8"
                ...
                oracle = expected[(syntax, case.label)]
                if oracle["decision"] != case.decision:
                    raise GateFailure(f"{syntax}/{case.label}: oracle decision changed")
                require_exact_backend(
                    syntax, case.label, "GLL", gll, oracle, source
                )
                require_exact_backend(
                    syntax, case.label, "GLR", glr, oracle, source
                )
                gll_exact += 1
                glr_exact += 1
                records.append(row_text(syntax, case.label, oracle))
                if case.decision == "accepted":
                    results = oracle["results"]
                    assert isinstance(results, tuple)
                    if len(results) != 1:
                        raise GateFailure(
                            f"{syntax}/{case.label}: expected one semantic result"
                        )
                    normalized[syntax] = normalize_result(
                        results[0], syntax, case.label
                    )
                    result_path = temp / f"result-{index}-{syntax}.term"
                    result_path.write_text(results[0], encoding="utf-8")
                    projections[syntax] = run_projection(
                        projection_binary,
                        projection_paths[syntax],
                        result_path,
                        f"{syntax}/{case.label}",
                    )
                elif oracle["results"]:
                    raise GateFailure(
                        f"{syntax}/{case.label}: rejected input has semantic results"
                    )
    ...
    if EXPECTED_MATRIX_DIGEST and digest != EXPECTED_MATRIX_DIGEST:
        raise GateFailure(f"rho syntax convergence matrix changed: {digest}")
    return (
        len(CASES),
        gll_exact,
        glr_exact,
