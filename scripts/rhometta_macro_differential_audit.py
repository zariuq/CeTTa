...
def run_text(cetta, src, macro_on):
    env = dict(os.environ)
    if not macro_on:
...
def runtime_stats_available(cetta):
...
def diverges(cetta, specs):
    case = quiet_farm_case(specs)
    a = mayset(run_text(cetta, render_run_source(case), True).stdout)
    b = mayset(run_text(cetta, render_run_source(case), False).stdout)
    return (a != b), a, b


def diverges_case(cetta, case):
    a = mayset(run_text(cetta, render_run_source(case), True).stdout)
    b = mayset(run_text(cetta, render_run_source(case), False).stdout)
    return (a != b), a, b
...
def stats_match(cetta, name, case):
    expected = stats_expectation(name)
    if expected is None or not runtime_stats_available(cetta):
        return True, None
    proc = run_text(cetta, render_stats_source(case), True)
...
def shrink(cetta, specs):
    ...
    while changed and len(specs) > 2:
        ...
        for i in range(len(specs)):
            ...
            if len(cand) >= 2 and diverges(cetta, cand)[0]:
...
def main():
    ...
    for name in NAMED_FIXTURES:
        ...
        div, ma, mb = diverges_case(cetta, case)
        if div:
            found += 1
            report(f"fixture {name}", case.expr, ma, mb)
        stats_ok, stats_msg = stats_match(cetta, name, case)
        if not stats_ok:
            found += 1
    ...
    for t in range(a.n):
        ...
        if div:
            ...
            if found <= 3 and family == "quiet":
                shrunk_specs = shrink(cetta, specs)
                shrunk_case = quiet_farm_case(shrunk_specs)
                _, sa, sb = diverges_case(cetta, shrunk_case)
