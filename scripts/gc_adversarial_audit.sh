...
for test_path in "${TESTS[@]}"; do
    ...
    baseline="$(run_case gc-on-budget-64 env CETTA_GC=1 CETTA_GC_BUDGET_MB=64 "$BIN" --profile he-extended --lang he "$test_path")"
    ...
    for budget in 1 2 8 64; do
        current="$(run_case "gc-on-budget-$budget" env CETTA_GC=1 CETTA_GC_BUDGET_MB="$budget" "$BIN" --profile he-extended --lang he "$test_path")"
    ...
    hashcons="$(run_case gc-on-eval-hashcons env CETTA_GC=1 CETTA_GC_BUDGET_MB=1 "$BIN" --eval-hashcons --profile he-extended --lang he "$test_path")"
