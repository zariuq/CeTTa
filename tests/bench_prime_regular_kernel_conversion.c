...
int main(int argc, char **argv) {
    const char *workload = argc > 1 ? argv[1] : "beta";
    ...
    if (strcmp(workload, "beta") == 0) {
        judgment_text =
            "(type:eq "
            " (App (Lam (Pi U0 U0) (idx 0)) (Lam U0 (idx 0)))"
            " (Lam U0 (idx 0)))";
        expected_status = "Established";
    } else if (strcmp(workload, "scoped-beta") == 0) {
        judgment_text =
            "(type:eq"
        ...
        expected_status = "Established";
    } else if (strcmp(workload, "eta") == 0) {
        judgment_text =
            "(type:eq"
            " (Lam (Pi U0 U0) (Lam U0 (App (idx 1) (idx 0))))"
            " (Lam (Pi U0 U0) (idx 0)))";
        expected_status = "Established";
    } else if (strcmp(workload, "scoped-eta") == 0) {
        judgment_text =
            "(type:eq"
            " (PrimeScoped PrimeCtxNil"
            "  (Lam (Pi U0 U0) (Lam U0 (App (idx 1) (idx 0)))))"
        ...
        expected_status = "Established";
    } else if (strcmp(workload, "distinct") == 0) {
        judgment_text = "(type:eq U0 (Pi U0 U0))";
        expected_status = "Refuted";
    } else if (strcmp(workload, "scoped-distinct") == 0) {
        judgment_text =
            "(type:eq (PrimeScoped PrimeCtxNil U0)"
            " (PrimeScoped PrimeCtxNil (Pi U0 U0)))";
        expected_status = "Refuted";
    } else if (strcmp(workload, "control") == 0) {
        judgment_text = "(type:eq 7 7)";
        expected_status = "Established";
    } else {
