...
int main(int argc, char **argv) {
    const char *workload = argc > 1 ? argv[1] : "identity";
    ...
    if (strcmp(workload, "identity") == 0) {
        judgment_text = "(type:check (Lam U0 (idx 0)) (Pi U0 U0))";
        expected_status = "Established";
    } else if (strcmp(workload, "scoped-identity") == 0) {
        judgment_text =
            "(type:check (PrimeScoped PrimeCtxNil (Lam U0 (idx 0)))"
            " (Pi U0 U0))";
        expected_status = "Established";
    } else if (strcmp(workload, "mismatch") == 0) {
        judgment_text = "(type:check (Lam U0 (idx 0)) U0)";
        expected_status = "Refuted";
    } else if (strcmp(workload, "scoped-mismatch") == 0) {
        judgment_text =
            "(type:check (PrimeScoped PrimeCtxNil (Lam U0 (idx 0))) U0)";
        expected_status = "Refuted";
    } else if (strcmp(workload, "control") == 0) {
        judgment_text = "(type:check 7 Number)";
        expected_status = "Established";
    } else {
