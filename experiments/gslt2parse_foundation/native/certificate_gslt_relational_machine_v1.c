...
typedef struct {
    ...
    PPCertificateGSLTRelationalDeclarationSnapshotV1 snapshot;
    bool snapshot_ready;
...
typedef struct {
    ...
    PPCertificateGSLTSequenceEvidenceProducerV1 evidence;
...
typedef struct {
    ...
    bool reused;
    bool full;
    PPCertificateGSLTRelationalDeclarationWorkspaceV1 declaration;
    PPCertificateGSLTRelationalPreparedWorkspaceV1 assertion_preparation;
...
static void ppproof_relational_machine_v1_clear_declaration_cache(
    ...
    for (index = 0u; index < entries->len; index++) {
        PPProofRelationalDeclarationCacheEntryV1 *entry =
            ...
            index;
        ppcertificate_gslt_relational_declaration_v1_free(&entry->declaration);
...
bool ppcertificate_gslt_relational_machine_v1_workspace_init(
    ...
    ppcertificate_gslt_sequence_evidence_producer_v1_init(&impl->evidence);
    if (!cetta_gslt_reusable_buffer_init_v1(
            &impl->stack,
            sizeof(PPProofRelationalMachineStackEntryV1)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->actuals,
            sizeof(PPCertificateGSLTRelationalActualHypothesisV1)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->required_binder_ids, sizeof(uint32_t)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->ordered_rows,
            sizeof(PPCertificateGSLTRelationalOrderedRowV1)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->binder_counts, sizeof(uint32_t)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->binding_schemas,
            sizeof(PPCertificateGSLTRelationalBindingSchemaV1)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->premise_schemas,
            sizeof(PPCertificateGSLTRelationalEssentialSchemaV1)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->apartness_pairs,
            sizeof(PPCertificateGSLTAssertionDisjointV1)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->ordered_premises,
            sizeof(PPCertificateGSLTRelationalOrderedHypothesisV1)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->prepared_bindings,
            sizeof(PPCertificateGSLTAssertionBindingV1)) ||
        !cetta_gslt_reusable_buffer_init_v1(
            &impl->prepared_premises,
            sizeof(PPCertificateGSLTAssertionEssentialV1)) ||
        !cetta_gslt_reusable_buffer_init_v1(
        ...
        cetta_gslt_repetition_admission_free_v1(
            &impl->declaration_admission);
        ppcertificate_gslt_sequence_evidence_producer_v1_free(&impl->evidence);
...
static PPCertificateGSLTRelationalMachineV1Result
ppproof_relational_machine_v1_workspace_acquire(
    ...
    if (!lease)
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID;
    memset(lease, 0, sizeof(*lease));
    if (!workspace)
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK;
    impl = workspace->impl;
    if (!impl)
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID;
    ...
    if (lock_result == EBUSY)
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK;
    if (lock_result != 0)
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_RESOURCE;
    lease->impl = impl;
    lease->full = full;
    ...
    lease->declaration = (PPCertificateGSLTRelationalDeclarationWorkspaceV1){
...
static void ppproof_relational_machine_v1_workspace_release(
    ...
    if (receipt) {
        PPCertificateGSLTSequenceEvidenceWorkspaceStatsV1 evidence_stats = {0};
...
ppproof_relational_machine_v1_from_article(
    ...
    switch (result) {
    case PPCERTIFICATE_GSLT_ARTICLE_V1_OK:
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK;
    case PPCERTIFICATE_GSLT_ARTICLE_V1_REJECTED:
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_REJECTED;
    case PPCERTIFICATE_GSLT_ARTICLE_V1_RESOURCE:
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_RESOURCE;
    case PPCERTIFICATE_GSLT_ARTICLE_V1_UNSUPPORTED:
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_UNSUPPORTED;
    case PPCERTIFICATE_GSLT_ARTICLE_V1_INVALID:
    default:
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID;
    }
}
...
static bool ppproof_relational_machine_v1_intern_selector(
    const PPRelationalStoreV1 *store,
    PPCertificateGSLTRelationalSelectorV1 selector,
    uint32_t *value_out) {
...
static PPCertificateGSLTNameV1 ppproof_relational_machine_v1_token_name(
    const PPCertificateGSLTSequenceTokenV1 *token) {
    if (!token || !token->term ||
        token->term->kind != PPCERTIFICATE_GSLT_PATTERN_V1_APPLY ||
        token->term->as.apply.argument_len != 0u)
        return (PPCertificateGSLTNameV1){0};
...
ppproof_relational_machine_v1_push_incomplete_claim(
    ...
    PPCertificateGSLTTokenSequenceV1 claim,
    PPProofRelationalMachineStackEntryV1 **stack,
    uint32_t *stack_len,
    uint32_t *stack_cap,
    char *error_buf,
    ...
    PPCertificateGSLTArticleV1Result article_result;

    article_result = ppcertificate_gslt_relational_context_v1_add_provable_premise(
        context, claim, &premise, error_buf, error_buf_size);
    if (article_result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK)
        article_result = ppcertificate_gslt_sequence_evidence_producer_v1_use_premise(
...
ppproof_relational_machine_v1_build_action_index(
    ...
    return cetta_gslt_u32_index_validate_v1(action_index)
               ? PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK
               : PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID;
}
...
static int ppproof_relational_machine_v1_frame_row_compare(
    ...
    const PPCertificateGSLTRelationalOrderedRowV1 *left_row = left;
    const PPCertificateGSLTRelationalOrderedRowV1 *right_row = right;
    ...
    for (;;) {
        ...
        if (!store->table_prefix_next(
                ...
                relational_plan->resolved_table_ids[
                    PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_ASSERTION_ACTIVE_HYPOTHESIS],
...
ppproof_relational_machine_v1_preload_frame(
    ...
    PPCertificateGSLTRelationalOrderedRowV1 *ordered = NULL;
    ...
    for (;;) {
        ...
        if (!store->table_prefix_next(
                store->context,
                relational_plan->resolved_table_ids[
                    PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_MANDATORY_VARIABLE],
                &authority, 1u, UINT32_C(1) << 1u,
                &cursor, row, 2u, &found))
            goto done;
        if (!found)
        ...
        if (mandatory_len >= maximum_len) {
            result = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_RESOURCE;
            goto done;
        }
        inserted = cetta_gslt_u32_index_insert_unique_v1(
    ...
    for (;;) {
        ...
        if (reuse_scratch) {
            PPCertificateGSLTRelationalOrderedRowV1 ordered_row = {
                position, row[2]
            };
            if (cetta_gslt_reusable_buffer_push_v1(
            ...
            ordered[ordered_len++] =
                (PPCertificateGSLTRelationalOrderedRowV1){position, row[2]};
        }
    }
    if (ordered_len > 1u)
    ...
    for (index = 0u; index < ordered_len; index++) {
        ...
        if (!store->table_find(
                store->context,
                relational_plan->resolved_table_ids[
                    PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_LABEL_KIND],
        ...
        if (kind_row[1] == selectors->floating_kind) {
            ...
            if (!store->table_find(
                    store->context,
                    relational_plan->resolved_table_ids[
                        PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_FLOATING_VARIABLE],
    ...
    result = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK;
    ...
    if (!ppproof_relational_machine_v1_intern_literal(
        ...
        !ppproof_relational_machine_v1_intern_selector(
            ...
            relational_plan->selectors[
                PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_HYPOTHESIS_FLOATING],
            &selectors.floating_kind) ||
        !ppproof_relational_machine_v1_intern_selector(
            store,
            relational_plan->selectors[
                PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_HYPOTHESIS_ESSENTIAL],
...
ppproof_relational_machine_v1_hypothesis(
    ...
    result = ppcertificate_gslt_relational_context_v1_formula(
        context, formula_row[1], &formula, error_buf, error_buf_size);
    if (result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK)
        result = ppcertificate_gslt_relational_context_v1_add_provable_premise(
...
ppproof_relational_machine_v1_declaration(
    PPCertificateGSLTRelationalContextV1 *context,
    uint32_t assertion,
    const PPCertificateGSLTRelationalDeclarationWorkspaceV1 *scratch,
    PPProofRelationalDeclarationCacheV1 *cache,
    uint32_t maximum_cache_len,
    PPCertificateGSLTRelationalDeclarationV1 *local,
    const PPCertificateGSLTRelationalDeclarationV1 **declaration_out,
    char *error_buf,
    size_t error_buf_size) {
    PPCertificateGSLTArticleV1Result elaborated;
    ...
    *declaration_out = NULL;
    if (!cache) {
        elaborated =
            ppcertificate_gslt_relational_declaration_v1_elaborate_with_workspace(
                ...
                error_buf, error_buf_size);
        if (elaborated != PPCERTIFICATE_GSLT_ARTICLE_V1_OK)
    ...
    if (admitted != CETTA_GSLT_REPETITION_OK_V1)
        return admitted == CETTA_GSLT_REPETITION_RESOURCE_V1
                   ? PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_RESOURCE
                   : PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID;
    if (decision == CETTA_GSLT_REPETITION_HIT_V1) {
        ...
        if (!cache->snapshot_ready ||
            !ppcertificate_gslt_relational_context_v1_declaration_snapshot_matches(
    ...
    if (cache->promotion_len == UINT32_MAX)
        return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_RESOURCE;
    if ((!cache->snapshot_ready &&
         !ppcertificate_gslt_relational_context_v1_declaration_snapshot(
             context, &cache->snapshot)) ||
...

    {
        ...
        ppcertificate_gslt_relational_declaration_v1_init(&entry.declaration);
        elaborated = ppcertificate_gslt_relational_declaration_v1_elaborate(
        ...
        if (pushed != CETTA_GSLT_REUSABLE_BUFFER_OK_V1) {
            ppcertificate_gslt_relational_declaration_v1_free(
                &entry.declaration);
            return pushed == CETTA_GSLT_REUSABLE_BUFFER_LIMIT_V1
                       ? PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_REJECTED
                       : PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_RESOURCE;
        }
...
ppproof_relational_machine_v1_assertion(
    ...
    const PPCertificateGSLTRelationalDeclarationWorkspaceV1 *declaration_workspace,
    const PPCertificateGSLTRelationalPreparedWorkspaceV1 *prepared_workspace,
    PPProofRelationalDeclarationCacheV1 *declaration_cache,
    ...
    char *error_buf,
    size_t error_buf_size) {
    PPCertificateGSLTRelationalDeclarationV1 local_schema;
    const PPCertificateGSLTRelationalDeclarationV1 *schema = NULL;
    PPCertificateGSLTRelationalPreparedAssertionV1 prepared;
    PPCertificateGSLTRelationalDeclarationStorageV1 declaration_control = {0};
    PPCertificateGSLTRelationalPreparedStorageV1 prepared_control = {0};
    PPCertificateGSLTRelationalDeclarationWorkspaceV1 declaration_call_workspace;
    PPCertificateGSLTRelationalPreparedWorkspaceV1 prepared_call_workspace;
    const PPCertificateGSLTRelationalDeclarationWorkspaceV1 *effective_declaration =
        declaration_workspace;
    const PPCertificateGSLTRelationalPreparedWorkspaceV1 *effective_prepared =
        prepared_workspace;
    PPCertificateGSLTRelationalActualHypothesisV1 *actuals = NULL;
    PPCertificateGSLTRelationalEvidenceV1 relational_evidence;
    PPCertificateGSLTSequenceEvidenceSourcesV1 sources;
    PPCertificateGSLTReferenceV1 declaration_reference;
    PPCertificateGSLTAssertionApplicationV1 application;
    PPCertificateGSLTTokenSequenceV1 formula;
    PPCertificateGSLTArticleV1Result result;
    PPCertificateGSLTRelationalMachineV1Result mapped;
    const char *failure_stage = "declaration elaboration";
    uint32_t stack_base;
    uint32_t index;

    ppcertificate_gslt_relational_declaration_v1_init(&local_schema);
    ppcertificate_gslt_relational_prepared_assertion_v1_init(&prepared);
    ...
    mapped = ppproof_relational_machine_v1_declaration(
        ...
        error_buf, error_buf_size);
    if (mapped != PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK) {
        if (mapped == PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID &&
    ...
    if (schema->ordered_len > *stack_len) {
        ...
        mapped = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_REJECTED;
        goto done;
    }
    if (actual_workspace) {
        if (!actual_workspace->active ||
            ...
            !cetta_gslt_reusable_buffer_truncate_v1(
                actual_workspace, 0u)) {
            mapped = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID;
            goto done;
        }
        ...
        if ((size_t)schema->ordered_len > SIZE_MAX / sizeof(*actuals) ||
            ...
            mapped = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_RESOURCE;
            goto done;
        }
    }
    stack_base = *stack_len - schema->ordered_len;
    for (index = 0u; index < schema->ordered_len; index++) {
        PPCertificateGSLTRelationalActualHypothesisV1 actual = {
    ...
    if (result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK) {
        ...
        result = ppcertificate_gslt_relational_context_v1_reserve_premise(
            context, &declaration_reference, error_buf, error_buf_size);
    }
    if (result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK) {
        failure_stage = "evidence-source construction";
        result = ppcertificate_gslt_relational_context_v1_evidence_sources(
    ...
    if (result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK) {
        ...
        result = ppcertificate_gslt_relational_context_v1_fill_premise(
            context, declaration_reference, application.declared_goal,
            error_buf, error_buf_size);
    }
    if (result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK) {
        failure_stage = "typed-result construction";
        result = ppcertificate_gslt_relational_context_v1_typed_formula(
    ...
    *entry_out = (PPProofRelationalMachineStackEntryV1){
    ...
    mapped = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK;

done:
    if (!actual_workspace)
        free(actuals);
    ppcertificate_gslt_relational_prepared_assertion_v1_free(&prepared);
    ppcertificate_gslt_relational_declaration_v1_free(&local_schema);
    return mapped;
}
...
ppproof_relational_machine_v1_apply_prepared_label(
    ...
    if (result != PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK) {
...
ppproof_relational_machine_v1_finish_incomplete(
    ...
    char *error_buf,
    size_t error_buf_size) {
    const PPCertificateGSLTPatternV1 *premises = NULL;
    uint32_t premise_len = 0u;
    PPCertificateGSLTArticleV1Result article_result;

    article_result = ppcertificate_gslt_relational_context_v1_view(
    ...
    return PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INCOMPLETE;
}
...
ppproof_relational_machine_v1_finish(
    ...
    PPCertificateGSLTArticleV1Receipt article_receipt;
    PPCertificateGSLTArticleV1Result article_result;
    ...
    if (!stack[0].goal ||
        stack[0].proof.kind != PPCERTIFICATE_GSLT_REFERENCE_V1_NODE ||
    ...
    if (!ppproof_relational_machine_v1_formula_equal(
        ...
        PPCertificateGSLTNameV1 actual = {0};
        PPCertificateGSLTNameV1 expected = {0};
    ...
    article_result = ppcertificate_gslt_article_v1_check_open(
...
ppcertificate_gslt_relational_machine_v1_normal_with_workspace(
    ...
    PPProofRelationalMachineSelectorsV1 selectors;
    PPCertificateGSLTRelationalContextV1 context;
    PPCertificateGSLTSequenceEvidenceProducerV1 producer_storage;
    PPCertificateGSLTSequenceEvidenceProducerV1 *producer = &producer_storage;
    ...
    PPCertificateGSLTTokenSequenceV1 claim;
    PPCertificateGSLTRelationalMachineV1Receipt receipt = {0};
    PPCertificateGSLTRelationalMachineV1Result result =
        PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID;
    PPCertificateGSLTArticleV1Result article_result;
    ...
    ppcertificate_gslt_relational_context_v1_init(&context);
    ppcertificate_gslt_sequence_evidence_producer_v1_init(&producer_storage);
    cetta_gslt_u32_index_init_v1(&local_action_index);
    receipt.execution =
        PPCERTIFICATE_GSLT_RELATIONAL_EXECUTION_V1_LABEL_STREAM;
    ...
    article_result = ppcertificate_gslt_relational_context_v1_begin(
        &context, store, relational_plan, evidence_abi, limits,
        error_buf, error_buf_size);
    if (article_result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK)
        article_result = ppcertificate_gslt_relational_context_v1_formula(
            &context, claim_value, &claim, error_buf, error_buf_size);
    if (article_result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK)
        article_result = ppcertificate_gslt_sequence_evidence_producer_v1_begin(
    ...
    for (index = 0u; index < input->step_len; index++) {
        ...
        if (ppproof_relational_machine_v1_slice_equals_literal(
            ...
            if (execution->unknown_policy !=
                ...
                result = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_REJECTED;
            } else {
                incomplete = true;
    ...
    if (result == PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID &&
        error_buf && error_buf_size != 0u && error_buf[0] == '\0') {
        ppproof_relational_machine_v1_set_error(
            error_buf, error_buf_size,
    ...
    if ((result == PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK ||
         result == PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INCOMPLETE) &&
        receipt_out)
        *receipt_out = receipt;
    free(stack);
    cetta_gslt_u32_index_free_v1(&local_action_index);
    ppcertificate_gslt_sequence_evidence_producer_v1_free(&producer_storage);
    ppcertificate_gslt_relational_context_v1_free(&context);
    return result;
}
...
ppcertificate_gslt_relational_machine_v1_normal(
    ...
    char *error_buf,
    size_t error_buf_size) {
    return ppcertificate_gslt_relational_machine_v1_normal_with_workspace(
...
typedef struct {
    ...
    bool *incomplete;
    uint32_t last_label;
    PPCertificateGSLTRelationalMachineV1Result last_result;
    char *error_buf;
    size_t error_buf_size;
...
ppproof_relational_machine_v1_effect_use_prepared(
    ...
    if (effects->prepared_use_len == UINT64_MAX) {
        effects->last_result = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_RESOURCE;
        return CETTA_GSLT_INDEXED_EFFECT_RESOURCE_V1;
    }
...
ppcertificate_gslt_relational_machine_v1_compressed_with_workspace(
    ...
    const PPCertificateGSLTArticleV1Limits *effective_limits = limits;
    const PPCertificateGSLTRelationalExecutionDescriptorV1 *execution;
    PPProofRelationalMachineSelectorsV1 selectors;
    PPCertificateGSLTRelationalContextV1 context;
    PPCertificateGSLTSequenceEvidenceProducerV1 producer_storage;
    ...
    cetta_gslt_u32_index_init_v1(&local_action_index);
    receipt.execution =
        PPCERTIFICATE_GSLT_RELATIONAL_EXECUTION_V1_INDEXED_INSTRUCTION_STREAM;
    if (!effective_limits) {
    ...
    effects = (PPProofRelationalIndexedEffectsV1){
        ...
        .last_result = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID,
        .error_buf = error_buf,
    ...
    for (index = 0u; index < input->code_len; index++) {
        ...
        if (!ppproof_relational_machine_v1_slice_valid(
            ...
            result = PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INVALID;
            goto done;
        }
...
ppcertificate_gslt_relational_machine_v1_compressed(
    ...
    char *error_buf,
    size_t error_buf_size) {
    return ppcertificate_gslt_relational_machine_v1_compressed_with_workspace(
        store, state_plan, proof_plan, evidence_abi,
