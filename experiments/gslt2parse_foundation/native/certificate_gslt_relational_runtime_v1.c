...
typedef struct {
    const char *row_relation;
    const char *count_relation;
    const char *table;
    ...
    const char *successor;
    uint32_t table_id;
} PPProofRuntimeBinaryTableEnumerationV1;
...
typedef struct {
    ...
    PPProofRuntimeBinaryTableEnumerationV1 *binary_table_enumerations;
    uint32_t binary_table_enumeration_len;
    ...
    PPCertificateGSLTRelationalRuntimeV1Receipt receipt;
    bool receipt_ready;
...
static void ppproof_runtime_v1_impl_free(PPProofRuntimeImplV1 *impl) {
    ...
    free(impl->binary_table_enumerations);
...
bool ppcertificate_gslt_relational_runtime_v1_attach_compiled_audit(
...
void ppcertificate_gslt_relational_runtime_v1_init(
    PPCertificateGSLTRelationalRuntimeV1 *runtime) {
    if (runtime)
        memset(runtime, 0, sizeof(*runtime));
}

void ppcertificate_gslt_relational_runtime_v1_free(
    PPCertificateGSLTRelationalRuntimeV1 *runtime) {
    if (!runtime)
        return;
...
bool ppcertificate_gslt_relational_runtime_v1_prepare(
    PPCertificateGSLTRelationalRuntimeV1 *runtime,
    ...
    uint32_t binary_table_enumeration_count = 0u;
    ...
    uint32_t binary_table_enumeration_index = 0u;
    ...
    for (answer_index = 0u; answer_index < impl->answers.len;
        ...
        if (ppproof_runtime_v1_expr_head(
            ...
            count_count++;
                       ...
                       record,
                       "proof-relational-runtime-distinct-pairs-v1", 4u)) {
            if (distinct_pair_count == UINT32_MAX)
                goto malformed;
            distinct_pair_count++;
        } else if (ppproof_runtime_v1_expr_head(
                       record,
                       "proof-relational-runtime-binary-table-enumeration-v1", 8u)) {
            if (binary_table_enumeration_count == UINT32_MAX)
                goto malformed;
            binary_table_enumeration_count++;
        } else if (ppproof_runtime_v1_expr_head(
                       record,
    ...
    impl->binary_table_enumerations = calloc(
        binary_table_enumeration_count ? binary_table_enumeration_count : 1u,
        sizeof(*impl->binary_table_enumerations));
    ...
    if (!impl->tables || !impl->ordinals || !impl->control_literals ||
        !impl->counts || !impl->distinct_pairs ||
        !impl->binary_table_enumerations || !impl->input_bytes ||
        ...
        if (ppproof_runtime_v1_expr_head(
            ...
            if (!ppproof_runtime_v1_owner(impl, record->expr.elems[1]) ||
                ...
                goto malformed;
                       ...
                       record,
                       "proof-relational-runtime-binary-table-enumeration-v1", 8u)) {
            PPProofRuntimeBinaryTableEnumerationV1 *enumeration =
                &impl->binary_table_enumerations[
                    binary_table_enumeration_index++];

            if (!ppproof_runtime_v1_owner(impl, record->expr.elems[1]) ||
                !(enumeration->row_relation = ppproof_runtime_v1_symbol(
                      record->expr.elems[2])) ||
                !(enumeration->count_relation = ppproof_runtime_v1_symbol(
                      record->expr.elems[3])) ||
                !(enumeration->table = ppproof_runtime_v1_symbol(
                      record->expr.elems[4])) ||
                !ppproof_runtime_v1_u32(
                    record->expr.elems[5], &enumeration->left_column) ||
                !ppproof_runtime_v1_u32(
                    record->expr.elems[6], &enumeration->right_column) ||
                !(enumeration->zero = ppproof_runtime_v1_symbol(
                      record->expr.elems[7])) ||
                !(enumeration->successor = ppproof_runtime_v1_symbol(
                      record->expr.elems[8])))
                goto malformed;
        } else if (ppproof_runtime_v1_expr_head(
                       record,
    ...
    impl->binary_table_enumeration_len = binary_table_enumeration_count;
    ...
    for (binary_table_enumeration_index = 0u;
         binary_table_enumeration_index <
             impl->binary_table_enumeration_len;
         binary_table_enumeration_index++) {
        PPProofRuntimeBinaryTableEnumerationV1 *enumeration =
            &impl->binary_table_enumerations[
                binary_table_enumeration_index];
        int32_t table_id = ppproof_runtime_v1_state_table(
            state_plan, enumeration->table);
        uint32_t prior;

        if (table_id < 0 ||
            enumeration->left_column >= state_plan->tables[table_id].arity ||
            enumeration->right_column >= state_plan->tables[table_id].arity ||
            enumeration->left_column == enumeration->right_column ||
            !ppproof_runtime_v1_external(
                native_plan, enumeration->row_relation, 4u) ||
            !ppproof_runtime_v1_external(
                native_plan, enumeration->count_relation, 2u) ||
            !ppproof_runtime_v1_head(
                native_plan, enumeration->zero, 0u) ||
            !ppproof_runtime_v1_head(
                native_plan, enumeration->successor, 1u))
            goto unsupported;
        for (prior = 0u; prior < binary_table_enumeration_index; prior++) {
            const PPProofRuntimeBinaryTableEnumerationV1 *existing =
                &impl->binary_table_enumerations[prior];
            if (existing->table_id == (uint32_t)table_id &&
                existing->left_column == enumeration->left_column &&
                existing->right_column == enumeration->right_column &&
                ppproof_runtime_v1_same(
                    existing->row_relation, enumeration->row_relation) &&
                ppproof_runtime_v1_same(
                    existing->count_relation,
                    enumeration->count_relation))
                goto malformed;
        }
        enumeration->table_id = (uint32_t)table_id;
    }
...
static bool ppproof_runtime_v1_binary_table_enumeration_rows(
    const PPProofRuntimeImplV1 *impl,
    const PPRelationalStoreV1 *store,
    PPProofRuntimeRowsV1 *rows,
    char *error_buf,
    ...
    for (descriptor_index = 0u;
         descriptor_index < impl->binary_table_enumeration_len;
         descriptor_index++) {
        const PPProofRuntimeBinaryTableEnumerationV1 *descriptor =
            &impl->binary_table_enumerations[descriptor_index];
        uint32_t arity = 0u;
        uint32_t key_arity = 0u;
        uint32_t row_len = 0u;
        ...
        if (!store->table_shape(
                store->context, descriptor->table_id,
                &arity, &key_arity, &row_len) ||
            descriptor->left_column >= arity ||
            descriptor->right_column >= arity ||
            (size_t)arity > SIZE_MAX / sizeof(*values))
            return ppproof_runtime_v1_fail(
                error_buf, error_buf_size,
                "generated binary-table enumeration no longer matches its state table");
        values = malloc((size_t)arity * sizeof(*values));
        owner = atom_symbol(&rows->arena, impl->owner);
        if (!values || !owner) {
            free(values);
            return ppproof_runtime_v1_fail(
                error_buf, error_buf_size,
                "generated binary-table enumeration exhausted resources");
        }
        for (row_index = 0u; row_index < row_len; row_index++) {
            Atom *arguments[4];
            Atom *left;
            ...
            if (!store->table_row(
                    store->context, descriptor->table_id, row_index,
                    values, arity) ||
                !(left = ppproof_runtime_v1_value(
                      impl, store, rows,
                      values[descriptor->left_column])) ||
                !(right = ppproof_runtime_v1_value(
                      impl, store, rows,
                      values[descriptor->right_column]))) {
                free(values);
                return ppproof_runtime_v1_fail(
                    error_buf, error_buf_size,
                    "generated binary-table enumeration could not decode a state row");
            }
            arguments[0] = owner;
            arguments[1] = ppproof_runtime_v1_constructor_chain(
                rows, descriptor->zero, descriptor->successor, row_index);
            arguments[2] = left;
            arguments[3] = right;
            if (!arguments[1] ||
                !ppproof_runtime_v1_rows_append(
                    rows, ppproof_runtime_v1_app(
                        &rows->arena, descriptor->row_relation,
                        arguments, 4u))) {
                free(values);
                return ppproof_runtime_v1_fail(
                    error_buf, error_buf_size,
                    "generated binary-table enumeration row vector exhausted resources");
            }
        }
        free(values);
        Atom *count = ppproof_runtime_v1_constructor_chain(
            rows, descriptor->zero, descriptor->successor, row_len);
        if (!ppproof_runtime_v1_rows_append(
                rows, ppproof_runtime_v1_binary(
                    &rows->arena, descriptor->count_relation,
                    owner, count)))
            return ppproof_runtime_v1_fail(
                error_buf, error_buf_size,
                "generated binary-table enumeration count exhausted resources");
    }
    return true;
}
...
static bool ppproof_runtime_v1_distinct_domain_contains(
    const PPProofRuntimeDistinctPairsV1 *pairs,
    const PPRelationalStoreV1 *store,
    uint32_t value,
    bool *contains_out) {
    ...
    *contains_out = false;
    ...
    for (row_index = 0u; row_index < row_len; row_index++) {
        ...
        if (!store->table_row(
                store->context, pairs->table_id, row_index,
                row_values, arity) ||
            !ppproof_runtime_v1_store_values_equal(
                store, row_values[pairs->value_column], value, &equal))
            goto done;
        if (equal) {
            *contains_out = true;
            break;
...
static bool ppproof_runtime_v1_append_exact_distinct_rows(
    ...
    for (descriptor_index = 0u;
        ...
        for (prior = 0u; prior < descriptor_index; prior++) {
            if (ppproof_runtime_v1_same(
                    impl->distinct_pairs[prior].relation,
                    pairs->relation))
                break;
        }
        if (prior < descriptor_index)
            continue;
        for (domain_index = 0u;
             domain_index < impl->distinct_pair_len; domain_index++) {
            const PPProofRuntimeDistinctPairsV1 *domain =
                &impl->distinct_pairs[domain_index];
            bool left_present = false;
            bool right_present = false;

            if (!ppproof_runtime_v1_same(domain->relation, pairs->relation))
                continue;
            if (!ppproof_runtime_v1_distinct_domain_contains(
                    domain, store, left_value, &left_present) ||
                !ppproof_runtime_v1_distinct_domain_contains(
                    domain, store, right_value, &right_present))
                return ppproof_runtime_v1_fail(
                    error_buf, error_buf_size,
                    "generated header admission could not inspect a distinct-value domain");
...
static bool ppproof_runtime_v1_compiled_audit(
    ...
    if (!agreed) {
        ...
        return ppproof_runtime_v1_fail(
            ...
            "pending-peak=%llu state-bytes-peak=%llu detail=%s",
            ...
            realization_error[0] ? realization_error : "<none>");
    }
    return true;
}
...
static PPRelationalStateProofV1Result ppproof_runtime_v1_execute(
    ...
    if (!ppproof_runtime_v1_persistent_cache_refresh(
        ...
        !ppproof_runtime_v1_distinct_rows(
            impl, request->store, &rows,
            error_buf, error_buf_size) ||
        !ppproof_runtime_v1_binary_table_enumeration_rows(
            impl, request->store, &rows,
            error_buf, error_buf_size) ||
...
PPRelationalStateProofV1Backend ppcertificate_gslt_relational_runtime_v1_backend(
    PPCertificateGSLTRelationalRuntimeV1 *runtime) {
...
bool ppcertificate_gslt_relational_runtime_v1_last_receipt(
    PPCertificateGSLTRelationalRuntimeV1 *runtime,
    PPCertificateGSLTRelationalRuntimeV1Receipt *receipt_out) {
    PPProofRuntimeImplV1 *impl;
...
bool ppcertificate_gslt_relational_runtime_v1_profile(
    const PPCertificateGSLTRelationalRuntimeV1 *runtime,
    PPCertificateGSLTRelationalRuntimeV1Profile *profile_out) {
    const PPProofRuntimeImplV1 *impl;
