...
static bool write_guarded_entries(const PPGuardedLexV1Plan *plan) {
    uint32_t index;

    for (index = 0u; index < plan->entry_len; index++) {
        const PPGuardedLexV1Entry *entry = &plan->entries[index];
        char *tag = render_term(entry->tag);
        char *guard_state = render_term(entry->guard_state);
        char *guard_body = render_term(entry->guard_body);
        int written = -1;

        if (tag && guard_state && guard_body) {
            written = printf(
                "guarded-lexical-entry\t%u\t%u\t%u\t%u\t%s\t%s\t%s\t%s\n",
                entry->tag_index,
                entry->state_id,
                entry->terminal_index,
...
static bool run(const char *abi_path,
    ...
    if (!ppnative_v1_start_is_closed_extended(
        ...
        (void)snprintf(
            error, sizeof(error),
            "guarded lexical ParserPack start remains open: %.432s",
    ...
    if (start_state_id < 0 || !cetta_lp_native_slr_summary(
        ...
        if (!error[0]) {
            (void)snprintf(
                error, sizeof(error),
                "failed to summarize guarded lexical SLR table");
        }
        goto rejected;
    }
    cetta_lp_native_grammar_summary(&grammar, &grammar_summary);
    printf("parser-pack-guarded-lexical-plan-v1\n");
    ...
    if (!write_tag_records("guard-tag", &guard_nfa) ||
        !write_guard_entries(&guard_plan)) {
        goto write_failed;
    }
    printf("guarded-compiler-digest\t%s\n", guarded_compiler_digest);
    printf("guarded-nfa-answer-digest\t%s\n", guarded_answers.digest);
    printf("guarded-nfa-answers\t%zu\n", guarded_answers.len);
    printf("guarded-tag-count\t%u\n", guarded_plan.prefix_nfa.nfa.tag_len);
    printf("guarded-plan-digest\t%s\n", guarded_plan.plan_digest);
    printf("guarded-terminal-extension-count\t%u\n",
           guarded_plan.entry_len);
    if (!write_tag_records(
            "guarded-tag", &guarded_plan.prefix_nfa) ||
        !write_guarded_entries(&guarded_plan)) {
        goto write_failed;
    }
...
write_failed:
    (void)snprintf(error, sizeof(error),
                   "failed to write guarded lexical plan stream");
rejected:
    fprintf(stderr, "guarded lexical plan stream rejected: %s\n",
...
int main(int argc, char **argv) {
    ...
    if (argc != 8) {
        fprintf(stderr,
                "usage: parser_pack_guarded_lexical_plan_v1_stream "
                "ABI LEXICAL_NFA GUARD_NFA GUARD_EVIDENCE GUARDED_NFA "
                "REGULAR_COMPILER_SHA256 GUARDED_COMPILER_SHA256\n");
        return 1;
    }
    ...
    g_hashcons = NULL;
    g_var_intern = NULL;
    ok = run(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7]);
