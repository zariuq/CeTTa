...
} PPProofGSLTPlanStorageV1;

typedef struct {
    PPProofGSLTNameV1 article;
    PPProofGSLTNameV1 name;
    const Atom *source;
    PPProofGSLTPatternV1 resolved;
    uint8_t resolution_state;
    bool used;
} PPProofGSLTPlanTermBindingV1;
...
typedef struct {
    ...
    bool used;
} PPProofGSLTPlanNodeBindingV1;

typedef struct {
    PPProofGSLTNameV1 article;
    const Atom *order;
    bool used;
} PPProofGSLTPlanOrderBindingV1;

typedef struct {
    PPProofGSLTNameV1 name;
    uint32_t index;
} PPProofGSLTPlanNodeIndexV1;

typedef struct {
    PPProofGSLTPlanTermBindingV1 *terms;
    uint32_t term_len;
    PPProofGSLTNameV1 article;
} PPProofGSLTPlanResolutionV1;

typedef struct {
    PPProofGSLTPlanStorageV1 *storage;
    const PPProofGSLTArticleV1Limits *limits;
    uint32_t remaining_patterns;
    PPProofGSLTArticleV1Result result;
    char *error_buf;
    size_t error_buf_size;
} PPProofGSLTPlanParseV1;

typedef enum {
    PPPROOF_GSLT_PLAN_RECORD_EXTENSION = 0,
    PPPROOF_GSLT_PLAN_RECORD_CONSTRUCTOR = 1,
    PPPROOF_GSLT_PLAN_RECORD_CALCULUS_CONSTRUCTOR = 2,
    PPPROOF_GSLT_PLAN_RECORD_JUDGMENT = 3,
    PPPROOF_GSLT_PLAN_RECORD_CAPABILITY = 4,
    PPPROOF_GSLT_PLAN_RECORD_CONVERSION = 5,
    PPPROOF_GSLT_PLAN_RECORD_RULE = 6,
    PPPROOF_GSLT_PLAN_RECORD_ARTICLE = 7,
    PPPROOF_GSLT_PLAN_RECORD_TERM_BINDING = 8,
    PPPROOF_GSLT_PLAN_RECORD_NODE_BINDING = 9,
    PPPROOF_GSLT_PLAN_RECORD_ORDER_BINDING = 10,
    PPPROOF_GSLT_PLAN_RECORD_DOCUMENT = 11,
    PPPROOF_GSLT_PLAN_RECORD_UNKNOWN = 12
} PPProofGSLTPlanRecordKindV1;
...
static void ppproof_plan_v1_fail(PPProofGSLTPlanParseV1 *parse,
                                 PPProofGSLTArticleV1Result result,
                                 const char *message) {
    if (parse->result == PPPROOF_GSLT_ARTICLE_V1_OK) {
        parse->result = result;
...
static void *ppproof_plan_v1_alloc(PPProofGSLTPlanParseV1 *parse,
                                   size_t count, size_t item_size) {
    PPProofGSLTPlanStorageV1 *storage = parse->storage;
    void **next;
    void *allocation;
    size_t next_cap;
    ...
    if (!ppproof_plan_v1_array_fits(count, item_size)) {
        ppproof_plan_v1_fail(
            parse, PPPROOF_GSLT_ARTICLE_V1_RESOURCE,
...
static bool ppproof_plan_v1_name_is(PPProofGSLTNameV1 name,
                                    const char *text) {
...
static int ppproof_plan_v1_name_compare(PPProofGSLTNameV1 left,
...
static int ppproof_plan_v1_term_binding_compare(const void *left_value,
    ...
    const PPProofGSLTPlanTermBindingV1 *left = left_value;
    const PPProofGSLTPlanTermBindingV1 *right = right_value;
...
static int ppproof_plan_v1_node_binding_compare(const void *left_value,
                                                 const void *right_value) {
    const PPProofGSLTPlanNodeBindingV1 *left = left_value;
    const PPProofGSLTPlanNodeBindingV1 *right = right_value;
    int compared = ppproof_plan_v1_name_compare(
        left->article, right->article);
...
static int ppproof_plan_v1_order_binding_compare(const void *left_value,
                                                  const void *right_value) {
    const PPProofGSLTPlanOrderBindingV1 *left = left_value;
    const PPProofGSLTPlanOrderBindingV1 *right = right_value;
...
static int ppproof_plan_v1_node_index_compare(const void *left_value,
                                               const void *right_value) {
    const PPProofGSLTPlanNodeIndexV1 *left = left_value;
    const PPProofGSLTPlanNodeIndexV1 *right = right_value;

    return ppproof_plan_v1_name_compare(left->name, right->name);
}

static PPProofGSLTPlanTermBindingV1 *ppproof_plan_v1_find_term_binding(
    PPProofGSLTPlanResolutionV1 *resolution,
    PPProofGSLTNameV1 name) {
    uint32_t low = 0u;
    ...
    while (low < high) {
        ...
        PPProofGSLTPlanTermBindingV1 *candidate =
...
static bool ppproof_plan_v1_nat(PPProofGSLTPlanParseV1 *parse,
                                const Atom *term,
                                uint32_t maximum,
                                uint32_t *out) {
    uint32_t value = 0u;
    ...
    while (!atom_is_symbol((Atom *)term, "proof-zero-v1")) {
        if (!ppproof_plan_v1_expr_head(term, "proof-succ-v1", 1u)) {
            ppproof_plan_v1_fail(
                parse, PPPROOF_GSLT_ARTICLE_V1_INVALID,
...
static bool ppproof_plan_v1_pattern_list_resolved(
    ...
    uint32_t traversal_depth,
    PPProofGSLTPlanResolutionV1 *resolution,
    PPProofGSLTPatternV1 **patterns_out,
    uint32_t *pattern_len_out) {
    PPProofGSLTPatternV1 *patterns = NULL;
...
static bool ppproof_plan_v1_pattern_resolved(
    ...
    if (ppproof_plan_v1_expr_head(
        ...
        PPProofGSLTNameV1 name;
        PPProofGSLTPlanTermBindingV1 *binding;
    ...
    if (ppproof_plan_v1_expr_head(term, "proof-bvar-v1", 1u)) {
        out->kind = PPPROOF_GSLT_PATTERN_V1_BVAR;
    ...
    if (ppproof_plan_v1_expr_head(term, "proof-fvar-v1", 1u)) {
        out->kind = PPPROOF_GSLT_PATTERN_V1_FVAR;
    ...
    if (ppproof_plan_v1_expr_head(term, "proof-lambda-v1", 1u)) {
        out->kind = PPPROOF_GSLT_PATTERN_V1_LAMBDA;
    ...
    if (ppproof_plan_v1_expr_head(
        ...
        out->kind = PPPROOF_GSLT_PATTERN_V1_MULTI_LAMBDA;
    ...
    if (ppproof_plan_v1_expr_head(term, "proof-subst-v1", 2u)) {
        out->kind = PPPROOF_GSLT_PATTERN_V1_SUBST;
    ...
    if (ppproof_plan_v1_expr_head(term, "proof-collection-v1", 2u)) {
        PPProofGSLTNameV1 kind;

        out->kind = PPPROOF_GSLT_PATTERN_V1_COLLECTION;
...
static bool ppproof_plan_v1_formals(
    PPProofGSLTPlanParseV1 *parse,
    const Atom *term,
    PPProofGSLTFormalV1 **formals_out,
    uint32_t *formal_len_out) {
    PPProofGSLTFormalV1 *formals = NULL;
    const Atom *cursor = term;
    uint32_t formal_len;
    uint32_t index;
...
static bool ppproof_plan_v1_conditions(
    PPProofGSLTPlanParseV1 *parse,
    const Atom *term,
    PPProofGSLTSideConditionV1 **conditions_out,
    uint32_t *condition_len_out) {
    PPProofGSLTSideConditionV1 *conditions = NULL;
    const Atom *cursor = term;
...
static bool ppproof_plan_v1_references(
    PPProofGSLTPlanParseV1 *parse,
    const Atom *term,
    PPProofGSLTReferenceV1 **references_out,
    uint32_t *reference_len_out) {
    PPProofGSLTReferenceV1 *references = NULL;
    const Atom *cursor = term;
    ...
    for (index = 0u; index < reference_len; index++) {
        ...
        if (ppproof_plan_v1_expr_head(
            ...
            references[index].kind =
                PPPROOF_GSLT_REFERENCE_V1_PREMISE;
        } else if (ppproof_plan_v1_expr_head(
                       reference, "proof-node-ref-v1", 1u)) {
            references[index].kind = PPPROOF_GSLT_REFERENCE_V1_NODE;
        } else {
            ppproof_plan_v1_fail(
                parse, PPPROOF_GSLT_ARTICLE_V1_INVALID,
...
static bool ppproof_plan_v1_nodes(
    PPProofGSLTPlanParseV1 *parse,
    const Atom *term,
    PPProofGSLTArticleNodeV1 **nodes_out,
    uint32_t *node_len_out) {
    PPProofGSLTArticleNodeV1 *nodes = NULL;
    const Atom *cursor = term;
    uint32_t node_len;
    uint32_t index;
    ...
    for (index = 0u; index < node_len; index++) {
        ...
        PPProofGSLTPatternV1 *arguments;
        uint32_t argument_len;
        PPProofGSLTReferenceV1 *references;
        uint32_t reference_len;
...
static bool ppproof_plan_v1_count_records(
    PPProofGSLTPlanParseV1 *parse,
    PPProofGSLTPlanRecordCountsV1 *counts) {
    size_t index;

    memset(counts, 0, sizeof(*counts));
    for (index = 0u; index < parse->storage->answers.len; index++) {
        const Atom *record = NULL;
        PPProofGSLTPlanRecordKindV1 kind = ppproof_plan_v1_record(
            parse->storage->answers.terms[index], &record);
        uint32_t *count = NULL;

        (void)record;
        switch (kind) {
        case PPPROOF_GSLT_PLAN_RECORD_EXTENSION:
            count = &counts->extension_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_CONSTRUCTOR:
            count = &counts->constructor_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_CALCULUS_CONSTRUCTOR:
            count = &counts->calculus_constructor_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_JUDGMENT:
            count = &counts->judgment_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_CAPABILITY:
            count = &counts->capability_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_CONVERSION:
            count = &counts->conversion_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_RULE:
            count = &counts->rule_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_ARTICLE:
            count = &counts->article_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_TERM_BINDING:
            count = &counts->term_binding_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_NODE_BINDING:
            count = &counts->node_binding_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_ORDER_BINDING:
            count = &counts->order_binding_len;
            break;
        case PPPROOF_GSLT_PLAN_RECORD_DOCUMENT:
...
static bool ppproof_plan_v1_owner(
    PPProofGSLTPlanParseV1 *parse,
    const Atom *term,
    PPProofGSLTNameV1 expected) {
    PPProofGSLTNameV1 actual;

    if (!ppproof_plan_v1_name(term, &actual) ||
        !ppproof_gslt_article_v1_name_equal(actual, expected)) {
        ppproof_plan_v1_fail(
            parse, PPPROOF_GSLT_ARTICLE_V1_INVALID,
...
static bool ppproof_plan_v1_capability(
    ...
    if (ppproof_plan_v1_name_is(name, "proof-cap-binders-v1"))
        *capability_out = PPPROOF_GSLT_ARTICLE_V1_CAP_BINDERS;
        ...
        *capability_out = PPPROOF_GSLT_ARTICLE_V1_CAP_COLLECTIONS;
    else if (ppproof_plan_v1_name_is(
...
static bool ppproof_plan_v1_rule(
    ...
    PPProofGSLTRuleSchemaV1 *rule) {
    PPProofGSLTFormalV1 *formals;
    PPProofGSLTPatternV1 *premises;
    PPProofGSLTPatternV1 *conclusion;
    PPProofGSLTSideConditionV1 *conditions;
...
static bool ppproof_plan_v1_article(
    ...
    const Atom *record,
    PPProofGSLTNameV1 owner,
    PPProofGSLTCompiledArticleV1 *compiled) {
    PPProofGSLTPatternV1 *context;
    PPProofGSLTArticleNodeV1 *nodes;
    PPProofGSLTPatternV1 *target;
    PPProofGSLTNameV1 policy;
...
static bool ppproof_plan_v1_node_binding(
    ...
    const Atom *record,
    PPProofGSLTNameV1 owner,
    PPProofGSLTPlanNodeBindingV1 *binding) {
    if (!ppproof_plan_v1_owner(parse, record->expr.elems[1], owner) ||
        !ppproof_plan_v1_name(
            record->expr.elems[2], &binding->article) ||
        !ppproof_plan_v1_name(
...
static bool ppproof_plan_v1_order_binding(
    ...
    const Atom *record,
    PPProofGSLTNameV1 owner,
    PPProofGSLTPlanOrderBindingV1 *binding) {
    if (!ppproof_plan_v1_owner(parse, record->expr.elems[1], owner) ||
        !ppproof_plan_v1_name(
            record->expr.elems[2], &binding->article)) {
        ppproof_plan_v1_fail(
...
static PPProofGSLTPlanNodeBindingV1 *ppproof_plan_v1_find_node_binding(
    PPProofGSLTPlanNodeBindingV1 *bindings,
    ...
    while (low < high) {
        uint32_t middle = low + (high - low) / 2u;
        PPProofGSLTPlanNodeBindingV1 *candidate = &bindings[middle];
        int compared = ppproof_plan_v1_name_compare(
...
static PPProofGSLTPlanOrderBindingV1 *ppproof_plan_v1_find_order_binding(
    PPProofGSLTPlanOrderBindingV1 *bindings,
    uint32_t binding_len,
    PPProofGSLTNameV1 article) {
    uint32_t low = 0u;
    uint32_t high = binding_len;
    ...
    while (low < high) {
        uint32_t middle = low + (high - low) / 2u;
        PPProofGSLTPlanOrderBindingV1 *candidate = &bindings[middle];
        int compared = ppproof_plan_v1_name_compare(
            article, candidate->article);
        if (compared < 0)
            high = middle;
...
static const PPProofGSLTPlanNodeIndexV1 *ppproof_plan_v1_find_node_index(
    const PPProofGSLTPlanNodeIndexV1 *indices,
    uint32_t index_len,
    PPProofGSLTNameV1 name) {
    uint32_t low = 0u;
    ...
    while (low < high) {
        uint32_t middle = low + (high - low) / 2u;
        const PPProofGSLTPlanNodeIndexV1 *candidate = &indices[middle];
        int compared = ppproof_plan_v1_name_compare(
            name, candidate->name);
        if (compared < 0)
            high = middle;
...
static bool ppproof_plan_v1_document_references(
    ...
    for (index = 0u; index < reference_len; index++) {
        ...
        if (ppproof_plan_v1_expr_head(
            ...
            PPProofGSLTNameV1 name;
            const PPProofGSLTPlanNodeIndexV1 *found;
            ...
            references[index].index = found->index;
                       ...
                       reference, "proof-premise-ref-v1", 1u)) {
            references[index].kind = PPPROOF_GSLT_REFERENCE_V1_PREMISE;
            if (!ppproof_plan_v1_nat(
                    parse, reference->expr.elems[1], UINT32_MAX,
                    &references[index].index))
                return false;
...
static bool ppproof_plan_v1_document(
    ...
    const Atom *record,
    PPProofGSLTNameV1 owner,
    PPProofGSLTPlanTermBindingV1 *term_bindings,
    uint32_t term_binding_len,
    PPProofGSLTPlanNodeBindingV1 *node_bindings,
    uint32_t node_binding_len,
    PPProofGSLTPlanOrderBindingV1 *order_bindings,
    uint32_t order_binding_len,
    PPProofGSLTCompiledArticleV1 *compiled) {
    PPProofGSLTPlanOrderBindingV1 *order_binding;
    PPProofGSLTPlanNodeIndexV1 *indices;
    PPProofGSLTNameV1 *ordered_names;
    PPProofGSLTArticleNodeV1 *nodes;
    PPProofGSLTPatternV1 *context;
    PPProofGSLTPatternV1 *target;
    PPProofGSLTPlanResolutionV1 resolution;
    PPProofGSLTNameV1 start;
    PPProofGSLTNameV1 root;
    PPProofGSLTNameV1 policy;
    const PPProofGSLTPlanNodeIndexV1 *root_index;
    const Atom *cursor;
    uint32_t node_len;
    uint32_t index;
    ...
    resolution = (PPProofGSLTPlanResolutionV1){
        .terms = term_bindings,
...
PPProofGSLTArticleV1Result ppproof_gslt_plan_v1_load(
    ...
    PPProofGSLTPlanV1 result_plan;
    PPProofGSLTPlanStorageV1 *storage = NULL;
    PPProofGSLTPlanParseV1 parse;
    PPProofGSLTPlanRecordCountsV1 counts;
    PPProofGSLTConstructorV1 *constructors = NULL;
    PPProofGSLTConstructorOriginV1 *constructor_origins = NULL;
    PPProofGSLTJudgmentV1 *judgments = NULL;
    PPProofGSLTRuleSchemaV1 *rules = NULL;
    PPProofGSLTCompiledArticleV1 *articles = NULL;
    PPProofGSLTPlanTermBindingV1 *term_bindings = NULL;
    PPProofGSLTPlanNodeBindingV1 *node_bindings = NULL;
    PPProofGSLTPlanOrderBindingV1 *order_bindings = NULL;
    ...
    for (index = 0u; index < storage->answers.len; index++) {
        ...
        if (ppproof_plan_v1_record(
            ...
            PPPROOF_GSLT_PLAN_RECORD_EXTENSION) {
            if (!ppproof_plan_v1_name(
    ...
    for (index = 0u; index < storage->answers.len; index++) {
        ...
        case PPPROOF_GSLT_PLAN_RECORD_CONSTRUCTOR: {
            ...
            constructor_origins[constructor_index] =
                PPPROOF_GSLT_CONSTRUCTOR_ORIGIN_V1_BASE;
            constructor_index++;
            break;
        }
        case PPPROOF_GSLT_PLAN_RECORD_CALCULUS_CONSTRUCTOR:
    ...
    parse.result = ppproof_gslt_article_v1_presentation_validate(
    ...
    for (index = 0u; index < result_plan.article_len; index++) {
        ...
        parse.result = ppproof_gslt_article_v1_check_open(
    ...
    ppproof_gslt_plan_v1_free(plan);
    *plan = result_plan;
    return PPPROOF_GSLT_ARTICLE_V1_OK;

failed:
    ppproof_plan_v1_storage_free(storage);
    return parse.result == PPPROOF_GSLT_ARTICLE_V1_OK
