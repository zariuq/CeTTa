...
typedef struct {
    ...
    PPProofGSLTArticleV1Result failure_result;
    bool schema_mode;
    char *error_buf;
...
typedef struct {
    ...
    bool judgment_root;
    bool finish;
} PPProofGSLTGroundValidationFrameV1;
...
typedef struct {
    ...
    bool judgment_root;
    bool occupied;
} PPProofGSLTGroundValidationCacheEntryV1;

typedef struct {
    PPProofGSLTGroundValidationCacheEntryV1 *entries;
    PPProofGSLTGroundValidationFrameV1 *frames;
    uint32_t len;
    uint32_t cap;
    uint32_t frame_cap;
    uint32_t maximum_entries;
} PPProofGSLTGroundValidationCacheV1;

typedef struct {
    uint32_t id;
    uint32_t index;
    bool occupied;
} PPProofGSLTArticleIdIndexEntryV1;

typedef struct {
    PPProofGSLTArticleIdIndexEntryV1 *entries;
    uint32_t cap;
} PPProofGSLTArticleIdIndexV1;

static void ppproof_gslt_v1_set_error(char *buf, size_t size,
                                      const char *format, ...) {
    va_list args;
...
PPProofGSLTArticleV1Limits ppproof_gslt_article_v1_default_limits(void) {
    return (PPProofGSLTArticleV1Limits){
...
bool ppproof_gslt_article_v1_name_equal(PPProofGSLTNameV1 left,
                                        PPProofGSLTNameV1 right) {
...
static bool ppproof_gslt_v1_name_well_formed(PPProofGSLTNameV1 name,
                                             bool allow_empty) {
    return (allow_empty || name.len != 0u) &&
           (name.len == 0u || name.bytes != NULL);
}

static bool ppproof_gslt_v1_array_well_formed(const void *items,
                                              uint32_t len) {
    return len == 0u || items != NULL;
}

static bool ppproof_gslt_v1_allocation_fits(uint32_t count,
                                           size_t item_size) {
    return count == 0u || item_size <= SIZE_MAX / (size_t)count;
}

static bool ppproof_gslt_v1_add_depth(uint32_t depth, uint32_t amount,
                                      uint32_t *out) {
...
static const PPProofGSLTConstructorV1 *ppproof_gslt_v1_find_constructor(
    const PPProofGSLTPresentationV1 *presentation,
...
static const PPProofGSLTJudgmentV1 *ppproof_gslt_v1_find_judgment(
    const PPProofGSLTPresentationV1 *presentation,
    PPProofGSLTNameV1 name) {
    uint32_t index;
...
static const PPProofGSLTRuleSchemaV1 *ppproof_gslt_v1_find_rule(
    const PPProofGSLTPresentationV1 *presentation,
...
static int32_t ppproof_gslt_v1_find_formal(
    const PPProofGSLTRuleSchemaV1 *rule,
    PPProofGSLTNameV1 name, uint32_t depth) {
    uint32_t index;
...
static bool ppproof_gslt_v1_validate_pattern(
    PPProofGSLTValidationV1 *validation,
    const PPProofGSLTPatternV1 *pattern,
    uint32_t ambient_depth,
    uint32_t traversal_depth,
    bool judgment_root);

static bool ppproof_gslt_v1_validate_pattern_array(
    PPProofGSLTValidationV1 *validation,
    const PPProofGSLTPatternV1 *patterns,
    uint32_t pattern_len,
    uint32_t ambient_depth,
    uint32_t traversal_depth) {
    uint32_t index;

    if (!ppproof_gslt_v1_array_well_formed(patterns, pattern_len)) {
    ...
    for (index = 0u; index < pattern_len; index++) {
        if (!ppproof_gslt_v1_validate_pattern(
...
static bool ppproof_gslt_v1_validate_pattern(
    ...
    if (traversal_depth > validation->maximum_depth) {
        validation->failure_result = PPPROOF_GSLT_ARTICLE_V1_RESOURCE;
        ppproof_gslt_v1_set_error(
            validation->error_buf, validation->error_buf_size,
    ...
    switch (pattern->kind) {
    case PPPROOF_GSLT_PATTERN_V1_BVAR:
        ...
        return true;
    case PPPROOF_GSLT_PATTERN_V1_FVAR:
        if (!validation->schema_mode ||
            !ppproof_gslt_v1_name_well_formed(pattern->as.fvar, false) ||
            ppproof_gslt_v1_find_formal(
        ...
        return true;
    case PPPROOF_GSLT_PATTERN_V1_APPLY: {
        uint32_t expected_arity = 0u;
        bool declared_shape = false;

        if (!ppproof_gslt_v1_name_well_formed(
        ...
        if (judgment_root) {
            ...
            const PPProofGSLTConstructorV1 *constructor =
                ppproof_gslt_v1_find_constructor(
                    validation->presentation,
                    pattern->as.apply.constructor);
            if (!constructor) {
                ppproof_gslt_v1_set_error(
        ...
        return ppproof_gslt_v1_validate_pattern_array(
    ...
    case PPPROOF_GSLT_PATTERN_V1_LAMBDA:
        ...
        if (pattern->as.lambda.binder_name_present ||
            ...
            !ppproof_gslt_v1_add_depth(ambient_depth, 1u, &inner_depth)) {
            ppproof_gslt_v1_set_error(
                validation->error_buf, validation->error_buf_size,
                "lambda pattern has noncanonical metadata or invalid depth");
            return false;
        }
        if (!ppproof_gslt_v1_add_depth(
                traversal_depth, 1u, &child_traversal_depth)) {
            validation->failure_result =
                PPPROOF_GSLT_ARTICLE_V1_RESOURCE;
            ppproof_gslt_v1_set_error(
        ...
        return ppproof_gslt_v1_validate_pattern(
            validation, pattern->as.lambda.body, inner_depth,
            child_traversal_depth, false);
    case PPPROOF_GSLT_PATTERN_V1_MULTI_LAMBDA:
        validation->used_capabilities |=
            PPPROOF_GSLT_ARTICLE_V1_CAP_BINDERS;
        ...
        return ppproof_gslt_v1_validate_pattern(
            ...
            child_traversal_depth, false);
    case PPPROOF_GSLT_PATTERN_V1_SUBST:
        validation->used_capabilities |=
    ...
    case PPPROOF_GSLT_PATTERN_V1_COLLECTION:
        ...
        if (pattern->as.collection.rest_present) {
            validation->used_capabilities |=
                PPPROOF_GSLT_ARTICLE_V1_CAP_COLLECTION_REST;
            validation->failure_result =
...
static bool ppproof_gslt_v1_pattern_contains_formal(
    const PPProofGSLTPatternV1 *pattern,
    PPProofGSLTNameV1 name,
    ...
    case PPPROOF_GSLT_PATTERN_V1_FVAR:
        return ambient_depth == target_depth &&
               ppproof_gslt_article_v1_name_equal(pattern->as.fvar, name);
    case PPPROOF_GSLT_PATTERN_V1_APPLY:
        for (index = 0u; index < pattern->as.apply.argument_len; index++) {
            if (ppproof_gslt_v1_pattern_contains_formal(
        ...
        return false;
    case PPPROOF_GSLT_PATTERN_V1_LAMBDA:
        return ppproof_gslt_v1_add_depth(
                   ambient_depth, 1u, &inner_depth) &&
               ppproof_gslt_v1_pattern_contains_formal(
...
static bool ppproof_gslt_v1_rule_contains_formal(
    const PPProofGSLTRuleSchemaV1 *rule,
    const PPProofGSLTFormalV1 *formal) {
    uint32_t index;
    ...
    return ppproof_gslt_v1_pattern_contains_formal(
        rule->conclusion, formal->name, formal->depth, 0u);
}

static bool ppproof_gslt_v1_names_unique_constructors(
    const PPProofGSLTPresentationV1 *presentation,
...
static bool ppproof_gslt_v1_names_unique_judgments(
    const PPProofGSLTPresentationV1 *presentation,
    char *error_buf, size_t error_buf_size) {
    uint32_t left;
    uint32_t right;
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_validate_rule(
    const PPProofGSLTPresentationV1 *presentation,
    const PPProofGSLTRuleSchemaV1 *rule,
    ...
    if (!ppproof_gslt_v1_name_well_formed(rule->id, false) ||
        !ppproof_gslt_v1_array_well_formed(rule->formals,
                                           rule->formal_len) ||
        !ppproof_gslt_v1_array_well_formed(rule->premises,
                                           rule->premise_len) ||
        !rule->conclusion ||
        !ppproof_gslt_v1_array_well_formed(rule->side_conditions,
    ...
    *capabilities_io = validation.used_capabilities;
    ...
    for (left = 0u; left < rule->formal_len; left++) {
        if (!ppproof_gslt_v1_rule_contains_formal(
    ...
    for (left = 0u; left < rule->side_condition_len; left++) {
        const PPProofGSLTSideConditionV1 *condition =
            &rule->side_conditions[left];
        const PPProofGSLTFormalV1 *body;
        const PPProofGSLTFormalV1 *result;
        const PPProofGSLTFormalV1 *replacement = NULL;
        uint32_t body_depth;
        ...
        case PPPROOF_GSLT_SIDE_CONDITION_V1_EXPLICIT_SUBSTITUTION:
            *capabilities_io |=
            ...
            break;
        case PPPROOF_GSLT_SIDE_CONDITION_V1_UNUSED_BINDER_ELIMINATION:
            *capabilities_io |=
...
PPProofGSLTArticleV1Result ppproof_gslt_article_v1_presentation_validate(
    ...
    if ((presentation->required_capabilities &
         ~PPPROOF_GSLT_ARTICLE_V1_SUPPORTED_CAPABILITIES) != 0u) {
        ppproof_gslt_v1_set_error(
            error_buf, error_buf_size,
            "presentation requires an unsupported capability");
        return PPPROOF_GSLT_ARTICLE_V1_UNSUPPORTED;
    }
    if (!ppproof_gslt_v1_names_unique_constructors(
            presentation, error_buf, error_buf_size) ||
        !ppproof_gslt_v1_names_unique_judgments(
            presentation, error_buf, error_buf_size))
        return PPPROOF_GSLT_ARTICLE_V1_INVALID;
    ...
    for (left = 0u; left < presentation->rule_len; left++) {
        ...
        result = ppproof_gslt_v1_validate_rule(
...
static void ppproof_gslt_v1_owned_pattern_clear(
    ...
    case PPPROOF_GSLT_PATTERN_V1_APPLY:
        ...
        for (index = 0u; index < pattern->as.apply.argument_len; index++)
            ppproof_gslt_v1_owned_pattern_clear(
                (PPProofGSLTPatternV1 *)&pattern->as.apply.arguments[index]);
    ...
    case PPPROOF_GSLT_PATTERN_V1_LAMBDA:
        if (!pattern->owns_children)
            break;
        ppproof_gslt_v1_owned_pattern_clear(
            (PPProofGSLTPatternV1 *)pattern->as.lambda.body);
    ...
    case PPPROOF_GSLT_PATTERN_V1_MULTI_LAMBDA:
        if (!pattern->owns_children)
            break;
        ppproof_gslt_v1_owned_pattern_clear(
            (PPProofGSLTPatternV1 *)pattern->as.multi_lambda.body);
    ...
    case PPPROOF_GSLT_PATTERN_V1_SUBST:
        if (!pattern->owns_children)
            break;
        ppproof_gslt_v1_owned_pattern_clear(
            (PPProofGSLTPatternV1 *)pattern->as.subst.body);
        ppproof_gslt_v1_owned_pattern_clear(
            (PPProofGSLTPatternV1 *)pattern->as.subst.replacement);
    ...
    case PPPROOF_GSLT_PATTERN_V1_COLLECTION:
        ...
        for (index = 0u; index < pattern->as.collection.element_len; index++)
            ppproof_gslt_v1_owned_pattern_clear(
                (PPProofGSLTPatternV1 *)&pattern->as.collection.elements[index]);
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_allocate_patterns(
    PPProofGSLTMaterialBudgetV1 *budget,
    uint32_t count,
    PPProofGSLTPatternV1 **out) {
    PPProofGSLTPatternV1 *items;

    *out = NULL;
    if (count == 0u)
        return PPPROOF_GSLT_ARTICLE_V1_OK;
    if (count > budget->remaining ||
        !ppproof_gslt_v1_allocation_fits(count, sizeof(*items)))
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_clone_pattern(
    const PPProofGSLTPatternV1 *source,
    ...
    if (!source || !target || !budget ||
        ...
        return PPPROOF_GSLT_ARTICLE_V1_RESOURCE;
    if (source->kind < PPPROOF_GSLT_PATTERN_V1_BVAR ||
        source->kind > PPPROOF_GSLT_PATTERN_V1_COLLECTION)
        return PPPROOF_GSLT_ARTICLE_V1_INVALID;
    *target = *source;
...
static bool ppproof_gslt_v1_pattern_equal_at(
    const PPProofGSLTPatternV1 *left,
    const PPProofGSLTPatternV1 *right,
    uint32_t traversal_depth,
    uint32_t maximum_depth) {
    uint32_t index;
    ...
    case PPPROOF_GSLT_PATTERN_V1_FVAR:
        return ppproof_gslt_article_v1_name_equal(
    ...
    case PPPROOF_GSLT_PATTERN_V1_MULTI_LAMBDA:
        ...
        return ppproof_gslt_v1_pattern_equal_at(
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_instantiate_pattern(
    const PPProofGSLTRuleSchemaV1 *rule,
    const PPProofGSLTPatternV1 *arguments,
    const PPProofGSLTPatternV1 *schema,
    uint32_t ambient_depth,
    PPProofGSLTPatternV1 *target,
    PPProofGSLTMaterialBudgetV1 *budget,
    uint32_t traversal_depth) {
    PPProofGSLTArticleV1Result result;
    PPProofGSLTPatternV1 *children = NULL;
    PPProofGSLTPatternV1 *body = NULL;
    PPProofGSLTPatternV1 *replacement = NULL;
    uint32_t index;
    uint32_t inner_depth;

    if (traversal_depth > budget->maximum_depth)
        return PPPROOF_GSLT_ARTICLE_V1_RESOURCE;
    if (schema->kind == PPPROOF_GSLT_PATTERN_V1_FVAR) {
        int32_t formal = ppproof_gslt_v1_find_formal(
            rule, schema->as.fvar, ambient_depth);
        if (formal < 0)
            return PPPROOF_GSLT_ARTICLE_V1_INVALID;
        return ppproof_gslt_v1_clone_pattern(
    ...
    case PPPROOF_GSLT_PATTERN_V1_APPLY:
        ...
        for (index = 0u; index < schema->as.apply.argument_len; index++) {
            result = ppproof_gslt_v1_instantiate_pattern(
                ...
                traversal_depth + 1u);
            if (result != PPPROOF_GSLT_ARTICLE_V1_OK) {
                ppproof_gslt_v1_owned_pattern_clear(target);
                return result;
            }
        }
        return PPPROOF_GSLT_ARTICLE_V1_OK;
    case PPPROOF_GSLT_PATTERN_V1_LAMBDA:
        if (!ppproof_gslt_v1_add_depth(
                ambient_depth, 1u, &inner_depth))
            return PPPROOF_GSLT_ARTICLE_V1_INVALID;
        result = ppproof_gslt_v1_allocate_patterns(budget, 1u, &body);
        if (result != PPPROOF_GSLT_ARTICLE_V1_OK)
            return result;
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_lift_pattern(
    const PPProofGSLTPatternV1 *source,
    uint32_t cutoff,
    uint32_t shift,
    PPProofGSLTPatternV1 *target,
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_transform_children_lift(
    const PPProofGSLTPatternV1 *sources,
    uint32_t source_len,
    uint32_t cutoff,
    uint32_t shift,
    PPProofGSLTPatternV1 **targets_out,
    PPProofGSLTMaterialBudgetV1 *budget,
    uint32_t traversal_depth) {
    PPProofGSLTPatternV1 *targets = NULL;
    PPProofGSLTArticleV1Result result;
    uint32_t index;
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_lift_pattern(
    ...
    case PPPROOF_GSLT_PATTERN_V1_APPLY:
        ...
        target->as.apply.arguments = NULL;
        target->as.apply.argument_len = 0u;
        result = ppproof_gslt_v1_transform_children_lift(
        ...
        return result;
    case PPPROOF_GSLT_PATTERN_V1_LAMBDA:
        if (!ppproof_gslt_v1_add_depth(cutoff, 1u, &inner_cutoff))
            return PPPROOF_GSLT_ARTICLE_V1_RESOURCE;
        result = ppproof_gslt_v1_allocate_patterns(budget, 1u, &body);
        if (result != PPPROOF_GSLT_ARTICLE_V1_OK)
            return result;
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_instantiate_bvar_at(
    const PPProofGSLTPatternV1 *source,
    uint32_t depth,
    const PPProofGSLTPatternV1 *replacement_source,
    PPProofGSLTPatternV1 *target,
    PPProofGSLTMaterialBudgetV1 *budget,
    uint32_t traversal_depth) {
    PPProofGSLTArticleV1Result result;
    ...
    case PPPROOF_GSLT_PATTERN_V1_BVAR:
        ...
        if (source->as.bvar == depth)
            return ppproof_gslt_v1_lift_pattern(
    ...
    case PPPROOF_GSLT_PATTERN_V1_APPLY:
        ...
        for (index = 0u; index < source->as.apply.argument_len; index++) {
            result = ppproof_gslt_v1_instantiate_bvar_at(
        ...
        return PPPROOF_GSLT_ARTICLE_V1_OK;
    case PPPROOF_GSLT_PATTERN_V1_LAMBDA:
        if (!ppproof_gslt_v1_add_depth(depth, 1u, &inner_depth))
            return PPPROOF_GSLT_ARTICLE_V1_RESOURCE;
        result = ppproof_gslt_v1_allocate_patterns(budget, 1u, &body);
        if (result != PPPROOF_GSLT_ARTICLE_V1_OK)
            return result;
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_drop_bvar_at(
    const PPProofGSLTPatternV1 *source,
    uint32_t cutoff,
    PPProofGSLTPatternV1 *target,
    PPProofGSLTMaterialBudgetV1 *budget,
    ...
    case PPPROOF_GSLT_PATTERN_V1_APPLY:
        ...
        target->owns_children = true;
        for (index = 0u; index < source->as.apply.argument_len; index++) {
            result = ppproof_gslt_v1_drop_bvar_at(
                &source->as.apply.arguments[index], cutoff,
                &children[index], budget, traversal_depth + 1u,
                dropped_out);
            if (result != PPPROOF_GSLT_ARTICLE_V1_OK || !*dropped_out) {
                ppproof_gslt_v1_owned_pattern_clear(target);
                return result;
            }
        }
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_side_conditions_hold(
    const PPProofGSLTRuleSchemaV1 *rule,
    const PPProofGSLTPatternV1 *arguments,
    PPProofGSLTMaterialBudgetV1 *budget,
    bool *holds_out) {
    ...
    *holds_out = false;
    for (index = 0u; index < rule->side_condition_len; index++) {
        ...
        const PPProofGSLTPatternV1 *result_argument =
            &arguments[condition->result_argument];
        PPProofGSLTPatternV1 transformed = {0};
        PPProofGSLTArticleV1Result result;
        bool dropped = true;
        bool equal;
        ...
        if (condition->kind ==
            PPPROOF_GSLT_SIDE_CONDITION_V1_EXPLICIT_SUBSTITUTION) {
            result = ppproof_gslt_v1_instantiate_bvar_at(
                body, 0u,
        ...
        if (result != PPPROOF_GSLT_ARTICLE_V1_OK)
            return result;
        if (!dropped) {
            ppproof_gslt_v1_owned_pattern_clear(&transformed);
            *holds_out = false;
            return PPPROOF_GSLT_ARTICLE_V1_OK;
        }
        equal = ppproof_gslt_v1_pattern_equal_at(
...
static size_t ppproof_gslt_v1_ground_validation_hash(
    const PPProofGSLTPatternV1 *pattern,
    uint32_t ambient_depth,
    bool judgment_root) {
...
static const PPProofGSLTGroundValidationCacheEntryV1 *
ppproof_gslt_v1_ground_validation_cache_find(
    const PPProofGSLTGroundValidationCacheV1 *cache,
    const PPProofGSLTPatternV1 *pattern,
    uint32_t ambient_depth,
    bool judgment_root) {
    size_t slot;
    ...
    if (!cache || cache->cap == 0u)
        return NULL;
    slot = ppproof_gslt_v1_ground_validation_hash(
               pattern, ambient_depth, judgment_root) &
           ((size_t)cache->cap - 1u);
    for (probes = 0u; probes < cache->cap; probes++) {
        const PPProofGSLTGroundValidationCacheEntryV1 *entry =
            &cache->entries[slot];
        if (!entry->occupied)
            return NULL;
...
static bool ppproof_gslt_v1_ground_validation_cache_insert_raw(
    PPProofGSLTGroundValidationCacheEntryV1 *entries,
    uint32_t cap,
    PPProofGSLTGroundValidationCacheEntryV1 value) {
    size_t slot = ppproof_gslt_v1_ground_validation_hash(
    ...
    for (probes = 0u; probes < cap; probes++) {
        PPProofGSLTGroundValidationCacheEntryV1 *entry = &entries[slot];
        if (!entry->occupied) {
            *entry = value;
            entry->occupied = true;
            return true;
...
static bool ppproof_gslt_v1_ground_validation_cache_grow(
    PPProofGSLTGroundValidationCacheV1 *cache) {
    PPProofGSLTGroundValidationCacheEntryV1 *grown;
    uint32_t next_cap;
    uint32_t index;

    next_cap = cache->cap == 0u ? 16u : cache->cap * 2u;
    if (next_cap < cache->cap ||
        !ppproof_gslt_v1_allocation_fits(next_cap, sizeof(*grown)))
        return false;
    ...
    for (index = 0u; index < cache->cap; index++) {
        if (cache->entries[index].occupied &&
            !ppproof_gslt_v1_ground_validation_cache_insert_raw(
...
static bool ppproof_gslt_v1_ground_validation_cache_add(
    PPProofGSLTGroundValidationCacheV1 *cache,
    const PPProofGSLTPatternV1 *pattern,
    uint32_t ambient_depth,
    bool judgment_root,
    ...
    if (ppproof_gslt_v1_ground_validation_cache_find(
    ...
    if (cache->cap == 0u ||
        ...
        if (!ppproof_gslt_v1_ground_validation_cache_grow(cache))
            return false;
    }
    if (!ppproof_gslt_v1_ground_validation_cache_insert_raw(
...
static bool ppproof_gslt_v1_ground_validation_cached_depth(
    const PPProofGSLTGroundValidationCacheV1 *cache,
    const PPProofGSLTPatternV1 *pattern,
    uint32_t ambient_depth,
    bool judgment_root,
...
static bool ppproof_gslt_v1_ground_validation_push(
    PPProofGSLTGroundValidationFrameV1 **frames,
    uint32_t *frame_len,
    uint32_t *frame_cap,
    uint32_t maximum_frames,
    PPProofGSLTGroundValidationFrameV1 frame) {
    PPProofGSLTGroundValidationFrameV1 *grown;
    uint32_t next_cap;
...
static bool ppproof_gslt_v1_ground_validation_relative_depth(
    const PPProofGSLTGroundValidationCacheV1 *cache,
    const PPProofGSLTPatternV1 *pattern,
    uint32_t ambient_depth,
    uint32_t *maximum_relative_depth_out) {
...
#define PP_ACCUMULATE_CHILD(child_pattern, child_ambient_depth)              \
    do {                                                                      \
        if (!ppproof_gslt_v1_ground_validation_cached_depth(                 \
    ...
    case PPPROOF_GSLT_PATTERN_V1_APPLY:
        ...
        break;
    case PPPROOF_GSLT_PATTERN_V1_LAMBDA:
        if (!ppproof_gslt_v1_add_depth(ambient_depth, 1u, &child_ambient))
            return false;
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_validate_ground_pattern(
    const PPProofGSLTPresentationV1 *presentation,
    const PPProofGSLTPatternV1 *pattern,
    uint32_t ambient_depth,
    bool judgment_root,
    ...
    PPProofGSLTArticleV1Result result = PPPROOF_GSLT_ARTICLE_V1_OK;
...
#define PP_GROUND_PUSH_KIND(                                                  \
    ...
    do {                                                                      \
        if (!ppproof_gslt_v1_ground_validation_push(                          \
...
#define PP_GROUND_FINISH(                                                     \
    ...
    while (frame_len != 0u) {
        ...
        const PPProofGSLTPatternV1 *current = frame.pattern;
        uint32_t child_depth;
        uint32_t inner_depth;
        uint32_t index;
        ...
        if (frame.finish) {
            if (!ppproof_gslt_v1_ground_validation_relative_depth(
            ...
            if (!ppproof_gslt_v1_ground_validation_cache_add(
                    cache, current, frame.ambient_depth,
        ...
        if (frame.judgment_root &&
            current->kind != PPPROOF_GSLT_PATTERN_V1_APPLY)
            PP_GROUND_REJECT(
                "judgment pattern root is not an application");
        ...
        case PPPROOF_GSLT_PATTERN_V1_COLLECTION:
            ...
            if (current->as.collection.rest_present) {
                ppproof_gslt_v1_set_error(
                    error_buf, error_buf_size,
                    "collection-rest patterns are not supported by this checker");
                result = PPPROOF_GSLT_ARTICLE_V1_UNSUPPORTED;
                goto done;
            }
        ...
        if (!has_children &&
            !ppproof_gslt_v1_ground_validation_cache_add(
                cache, current, frame.ambient_depth,
...
static bool ppproof_gslt_v1_article_id_index_find(
    const PPProofGSLTArticleIdIndexV1 *id_index,
    uint32_t id,
    uint32_t *index_out) {
    size_t slot;
    uint32_t probes;
    ...
    for (probes = 0u; probes < id_index->cap; probes++) {
        const PPProofGSLTArticleIdIndexEntryV1 *entry =
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_article_id_index_build(
    const PPProofGSLTArticleV1 *article,
    PPProofGSLTArticleIdIndexV1 *id_index) {
    ...
    for (index = 0u; index < article->node_len; index++) {
        ...
        while (id_index->entries[slot].occupied)
            slot = (slot + 1u) & ((size_t)cap - 1u);
        id_index->entries[slot] = (PPProofGSLTArticleIdIndexEntryV1){
...
static PPProofGSLTArticleV1Result ppproof_gslt_v1_article_rooted(
    const PPProofGSLTArticleV1 *article,
    const PPProofGSLTArticleIdIndexV1 *id_index,
    bool *rooted_out) {
    uint8_t *state = NULL;
...
PPProofGSLTArticleV1Result ppproof_gslt_article_v1_check_open(
    ...
    PPProofGSLTArticleV1Limits default_limits;
    const PPProofGSLTArticleV1Limits *limits = limits_argument;
    PPProofGSLTArticleV1Receipt receipt = {0};
    ...
    if (!article || article->version != 1u || !article->target ||
        !ppproof_gslt_v1_array_well_formed(context, context_len) ||
        !ppproof_gslt_v1_array_well_formed(article->nodes,
                                           article->node_len) ||
        article->node_len == 0u) {
        ppproof_gslt_v1_set_error(
            error_buf, error_buf_size,
    ...
    material_budget = (PPProofGSLTMaterialBudgetV1){
    ...
    for (index = 0u; index < article->node_len; index++) {
        ...
        rule = ppproof_gslt_v1_find_rule(
    ...
    if (result == PPPROOF_GSLT_ARTICLE_V1_RESOURCE &&
