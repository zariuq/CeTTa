...
static bool run_database(
    ...
    PPCertificateGSLTRelationalRuntimeV1 *proof_runtime,
    ...
    PPCertificateGSLTRelationalRuntimeV1Receipt proof_receipt;
    PPCertificateGSLTRelationalRuntimeV1Profile profile_before;
    PPCertificateGSLTRelationalRuntimeV1Profile profile_after;
    char error[512] = {0};
    bool executed;
    bool initialized;
    ...
    if (!expect(read_file(path, &bytes, &byte_len),
        ...
        goto done;
    proof_backend = ppcertificate_gslt_relational_runtime_v1_backend(proof_runtime);
    if (!expect(proof_backend.execute != NULL,
                "prepared generated proof provider has no backend"))
        goto done;
    if (!expect(ppcertificate_gslt_relational_runtime_v1_profile(
    ...
    receipt_available = ppcertificate_gslt_relational_runtime_v1_last_receipt(
...
int main(int argc, char **argv) {
    ...
    if (argc != 21) {
        fprintf(stderr,
                ...
                "POSITIVE-DISJOINT-NORMAL-MM "
                "POSITIVE-DISJOINT-COMPRESSED-MM "
                "MISSING-DISJOINT-NORMAL-MM "
                "MISSING-DISJOINT-COMPRESSED-MM\n",
                argv[0]);
        return 2;
    }
    ...
    ppcertificate_gslt_relational_runtime_v1_init(&proof_runtime);
    ...
    if (!expect(initialized, error[0] ? error :
        ...
        goto done;
    initialized = ppcertificate_gslt_relational_runtime_v1_prepare(
    ...
    initialized = ppcertificate_gslt_relational_runtime_v1_attach_compiled_audit(
    ...
    ok = run_database(argv[3], &program, &fold, &span_mask, &state,
         ...
         run_database(argv[16], &program, &fold, &span_mask, &state,
                      &proof_runtime, PPRELATIONAL_STATE_PROOF_V1_REJECTED,
                      0u, true) &&
         run_database(argv[17], &program, &fold, &span_mask, &state,
                      &proof_runtime, PPRELATIONAL_STATE_PROOF_V1_VERIFIED,
                      1u, false) &&
         run_database(argv[18], &program, &fold, &span_mask, &state,
                      &proof_runtime, PPRELATIONAL_STATE_PROOF_V1_VERIFIED,
                      1u, false) &&
         run_database(argv[19], &program, &fold, &span_mask, &state,
                      &proof_runtime, PPRELATIONAL_STATE_PROOF_V1_REJECTED,
                      0u, false) &&
         run_database(argv[20], &program, &fold, &span_mask, &state,
                      &proof_runtime, PPRELATIONAL_STATE_PROOF_V1_REJECTED,
                      0u, false) &&
    ...
    ppcertificate_gslt_relational_runtime_v1_free(&proof_runtime);
    ...
    printf("(CertificateGSLTRelationalRuntimeV1Summary %u %u %u)\n",
