...
static void check_plan(const char *path,
    ...
    const PPProofGSLTCompiledArticleV1 *article;
    ...
    expect_true(
        ...
        NULL);
    article = ppproof_gslt_plan_v1_find_article(&plan, article_id);
    expect_true("compiled-plan-article-lookup", article != NULL, NULL);
    if (article) {
        memset(error, 0, sizeof(error));
        result = ppproof_gslt_article_v1_check_open(
    ...
    expect_true("compiled-plan-foreign-article-absent",
                ppproof_gslt_plan_v1_find_article(
...
static void check_binder_details(const char *path) {
    ...
    PPProofGSLTPlanV1 plan;
    const PPProofGSLTCompiledArticleV1 *article;
    const PPProofGSLTRuleSchemaV1 *substitution = NULL;
    PPProofGSLTArticleV1Receipt receipt;
    PPProofGSLTArticleV1Result result;
    uint32_t index;
    char error[512] = {0};
    ...
    expect_true(
        ...
        NULL);
    article = ppproof_gslt_plan_v1_find_article(
...
int main(int argc, char **argv) {
    ...
    printf("(ProofGSLTPlanV1Summary %u %u %u)\n",
