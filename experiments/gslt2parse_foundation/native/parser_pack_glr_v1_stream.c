...
static bool run(const char *abi_path,
    ...
    if (!ppabi_v1_wire_read(
        ...
        !read_input(input_path, &input, &input_len,
                    error, sizeof(error)) ||
        !ppglr_v1_parse(
            &pack, start, input, input_len, fact_limit,
            replay_depth, result_limit,
            &result, error, sizeof(error))) {
        fprintf(stderr, "ParserPack GLR stream rejected: %s\n",
                error[0] ? error : "unknown rejection");
        goto done;
    }
    printf("parser-pack-glr-stream-v1\n");
    ...
    if (result.outcome == PPNATIVE_V1_COMPLETED) {
        ...
        printf("stack-nodes\t%u\n", result.forest.stack_node_len);
        for (result_index = 0u;
...
int main(int argc, char **argv) {
    ...
    g_hashcons = NULL;
    g_var_intern = NULL;
    ok = run(argv[1], argv[2], argv[3], fact_limit,
             replay_depth, result_limit);
