...
enum { TEST_TABLE_LEN = PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_LEN };
...
static bool setup_machine_rows(
    FakeStoreV1 *store,
    const PPCertificateGSLTRelationalAssertionPlanV1 *bridge,
    ...
    machine->symbol_kind_table = bridge->resolved_table_ids[
        PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_SYMBOL_KIND];
    machine->binder_hypothesis_kind = (PPRelationalStateLiteralV1){
        .bytes = (uint8_t *)bridge->selectors[
            PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_HYPOTHESIS_FLOATING].bytes,
        .len = bridge->selectors[
            PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_HYPOTHESIS_FLOATING].len,
    };
    machine->matching_hypothesis_kind = (PPRelationalStateLiteralV1){
        .bytes = (uint8_t *)bridge->selectors[
            PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_HYPOTHESIS_ESSENTIAL].bytes,
        .len = bridge->selectors[
            PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_HYPOTHESIS_ESSENTIAL].len,
    };
    ...
    machine->variable_symbol_kind = (PPRelationalStateLiteralV1){
        .bytes = (uint8_t *)bridge->selectors[
            PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_SYMBOL_VARIABLE].bytes,
        .len = bridge->selectors[
            PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_SYMBOL_VARIABLE].len,
    };
...
static bool setup_store(
    ...
    uint32_t selectors[PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_LEN];
    ...
    if (!add_pair(store, bridge->resolved_table_ids[
                       PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_SYMBOL_KIND],
                  tc, selectors[
                          PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_SYMBOL_LITERAL],
                  NULL) ||
...
static bool setup_repeated_identity(
    ...
    if (!store || !bridge ||
        !fake_value_intern(
            store,
            bridge->selectors[
                PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_SYMBOL_LITERAL].bytes,
            bridge->selectors[
                PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_SYMBOL_LITERAL].len,
            &literal_selector) ||
        !fake_value_intern(
            store,
            bridge->selectors[
...
static PPCertificateGSLTArticleV1Result check_application(
    ...
    result = ppcertificate_gslt_relational_context_v1_view(
...
int main(int argc, char **argv) {
    ...
    PPCertificateGSLTSequenceEvidenceABIV1 evidence_abi;
    ...
    PPCertificateGSLTRelationalContextV1 context;
    PPCertificateGSLTRelationalDeclarationV1 schema;
    PPCertificateGSLTRelationalPreparedAssertionV1 prepared;
    PPCertificateGSLTRelationalActualHypothesisV1 actuals[3];
    ...
    if (argc != 5 + PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_LEN) {
        fprintf(stderr,
    ...
    ppcertificate_gslt_sequence_evidence_abi_v1_init(&evidence_abi);
    ppcertificate_gslt_relational_assertion_v1_init(&bridge);
    ppcertificate_gslt_relational_context_v1_init(&context);
    ppcertificate_gslt_relational_declaration_v1_init(&schema);
    ...
    if (result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK &&
        ...
        if (result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK)
            result = ppcertificate_gslt_relational_context_v1_begin(
          ...
          "generic relational declaration context starts");
    {
        ...
        check(!ppcertificate_gslt_relational_context_v1_declaration_snapshot_matches(
                  &context, &snapshot),
    ...
    }
    {
        ...
        PPCertificateGSLTRelationalDeclarationWorkspaceV1 workspace = {
        ...
        PPCertificateGSLTRelationalDeclarationWorkspaceV1 partial_workspace = {
            .required_binder_ids = &required_binders,
        };
        PPCertificateGSLTRelationalDeclarationV1 workspace_schema;
        PPCertificateGSLTArticleV1Result workspace_result;
        ...
        ppcertificate_gslt_relational_declaration_v1_init(&workspace_schema);
        ...
        check(workspace_result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK &&
        ...
        ppcertificate_gslt_relational_declaration_v1_free(&workspace_schema);
        workspace_result = workspace_ready
            ? ppcertificate_gslt_relational_declaration_v1_elaborate_with_workspace(
                  &context, rule, &partial_workspace, &workspace_schema,
                  error, sizeof(error))
            : PPCERTIFICATE_GSLT_ARTICLE_V1_RESOURCE;
        check(workspace_result == PPCERTIFICATE_GSLT_ARTICLE_V1_INVALID,
    ...
    if (result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK)
        result = ppcertificate_gslt_relational_declaration_v1_elaborate(
    ...
    }
    {
        ...
        PPCertificateGSLTRelationalPreparedWorkspaceV1 workspace = {
            .bindings = &prepared_bindings,
            .premises = &prepared_premises,
        };
        PPCertificateGSLTRelationalPreparedWorkspaceV1 partial_workspace = {
            .bindings = &prepared_bindings,
        };
        PPCertificateGSLTRelationalPreparedAssertionV1 workspace_prepared;
        PPCertificateGSLTArticleV1Result workspace_result;
        bool workspace_ready =
            cetta_gslt_reusable_buffer_init_v1(
                &prepared_bindings,
        ...
        ppcertificate_gslt_relational_prepared_assertion_v1_init(
            &workspace_prepared);
        workspace_result =
            result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK && workspace_ready
        ...
        ppcertificate_gslt_relational_prepared_assertion_v1_free(
            &workspace_prepared);
        workspace_result = workspace_ready
            ? ppcertificate_gslt_relational_prepared_assertion_v1_build_with_workspace(
    ...
    if (result == PPCERTIFICATE_GSLT_ARTICLE_V1_OK)
        result = ppcertificate_gslt_relational_prepared_assertion_v1_build(
...

    {
        ...
        PPCertificateGSLTSequenceEvidenceProducerV1 negative_producer;
        PPCertificateGSLTAssertionApplicationV1 negative_application;

        row[0] = rule;
        ppcertificate_gslt_sequence_evidence_producer_v1_init(
            &negative_producer);
        result = ppcertificate_gslt_sequence_evidence_producer_v1_begin(
        ...
        ppcertificate_gslt_sequence_evidence_producer_v1_free(
            &negative_producer);
        row[0] = saved_left;
    }

    {
        FakeTableV1 *symbol_table = &fake_store.tables[
            bridge.resolved_table_ids[PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_SYMBOL_KIND]];
        uint32_t *kind_row = symbol_table->rows +
            (size_t)formal_x_kind_row * symbol_table->arity;
        uint32_t saved_kind = kind_row[1];
        PPCertificateGSLTRelationalContextV1 negative_context;
        PPCertificateGSLTRelationalDeclarationV1 negative_schema;

        kind_row[1] = symbol_table->rows[1];
        ppcertificate_gslt_relational_context_v1_init(&negative_context);
        ppcertificate_gslt_relational_declaration_v1_init(&negative_schema);
        result = ppcertificate_gslt_relational_context_v1_begin(
        ...
        ppcertificate_gslt_relational_declaration_v1_free(&negative_schema);
        ppcertificate_gslt_relational_context_v1_free(&negative_context);
...

    {
        ...
        PPCertificateGSLTRelationalNormalInputV1 machine_input = {
            .label = {
        ...
        PPCertificateGSLTRelationalMachineV1Receipt machine_receipt;
        PPCertificateGSLTRelationalMachineV1Receipt normal_receipt;
        PPCertificateGSLTRelationalMachineV1Result machine_result;
        PPCertificateGSLTRelationalMachineV1Workspace workspace = {0};
        bool workspace_ready =
            ppcertificate_gslt_relational_machine_v1_workspace_init(&workspace) &&
        ...
        machine_result = ppcertificate_gslt_relational_machine_v1_normal(
            &store, &state_plan, &proof_plan, &evidence_abi,
            &bridge, action_cases, 4u, &machine_input, NULL, &machine_receipt,
            error, sizeof(error));
        if (machine_result != PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK)
            fprintf(stderr, "normal machine diagnostic: %s\n", error);
        check(machine_result == PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_OK &&
        ...
        if (workspace_ready) {
...

            {
                ...
                PPCertificateGSLTRelationalNormalInputV1 identity_input = {
                    .label = {
...

        {
            ...
            PPCertificateGSLTRelationalCompressedInputV1 compressed_input = {
            ...
            machine_result =
                ppcertificate_gslt_relational_machine_v1_compressed(
                    ...
                    NULL, &machine_receipt, error, sizeof(error));
            check(machine_result ==
                      PPCERTIFICATE_GSLT_RELATIONAL_MACHINE_V1_INCOMPLETE &&
                      !machine_receipt.complete &&
                      machine_receipt.execution ==
                          PPCERTIFICATE_GSLT_RELATIONAL_EXECUTION_V1_INDEXED_INSTRUCTION_STREAM &&
        ...
        if (workspace_ready)
            ppcertificate_gslt_relational_machine_v1_workspace_free(&workspace);
    }

    {
        PPCertificateGSLTRelationalAssertionPlanV1 no_apartness_bridge;
        PPCertificateGSLTRelationalContextV1 no_apartness_context;
        PPCertificateGSLTRelationalEvidenceV1 no_apartness_evidence;
        PPCertificateGSLTSequenceEvidenceSourcesV1 no_apartness_sources;

        ppcertificate_gslt_relational_assertion_v1_init(&no_apartness_bridge);
        ppcertificate_gslt_relational_context_v1_init(&no_apartness_context);
        result = ppcertificate_gslt_relational_assertion_v1_load(
        ...
        ppcertificate_gslt_relational_context_v1_free(&no_apartness_context);
        ppcertificate_gslt_relational_assertion_v1_free(&no_apartness_bridge);
    }
    ...
    ppcertificate_gslt_relational_declaration_v1_free(&schema);
    ...
    ppcertificate_gslt_sequence_evidence_abi_v1_free(&evidence_abi);
