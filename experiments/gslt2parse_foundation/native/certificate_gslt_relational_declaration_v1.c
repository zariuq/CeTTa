...
typedef struct {
    uint32_t value;
    PPCertificateGSLTSequenceTokenV1 *token;
...
typedef struct {
    ...
    uint32_t selector_values[PPCERTIFICATE_GSLT_RELATIONAL_SELECTOR_V1_LEN];
...
static PPCertificateGSLTPatternV1 *ppproof_relational_context_v1_apply(
    PPProofRelationalContextImplV1 *impl,
...
static bool ppproof_relational_context_v1_grow_premises(
    ...
    PPCertificateGSLTPatternV1 *next_premises;
    bool *next_filled;
    uint32_t next_capacity;
...
static bool ppproof_relational_context_v1_table_shape(
    ...
    if (!impl || role >= PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_LEN ||
...
static bool ppproof_relational_context_v1_find(
    ...
    return impl && role < PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_LEN &&
...
static PPCertificateGSLTArticleV1Result
ppproof_relational_context_v1_token(
    PPProofRelationalContextImplV1 *impl,
    uint32_t value,
    const PPCertificateGSLTSequenceTokenV1 **token_out,
    char *error_buf,
    ...
    PPCertificateGSLTPatternV1 *classification;
    ...
    if (impl->token_len >=
        ...
        !ppproof_relational_context_v1_find(
            impl, PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_SYMBOL_KIND,
    ...
    term = ppproof_relational_context_v1_apply(
        impl, (PPCertificateGSLTNameV1){bytes, byte_len}, NULL, 0u);
    ...
    judgment = impl->evidence_abi->judgments[
        literal ? PPCERTIFICATE_GSLT_SEQUENCE_JUDGMENT_V1_LITERAL
                : PPCERTIFICATE_GSLT_SEQUENCE_JUDGMENT_V1_VARIABLE];
    arguments[0] = term;
...
PPCertificateGSLTArticleV1Result ppcertificate_gslt_relational_context_v1_begin(
    ...
    for (index = 0u;
        ...
        if (!ppproof_relational_context_v1_table_shape(
                impl, (PPCertificateGSLTRelationalTableRoleV1)index,
    ...
    for (index = 0u;
        ...
        const PPCertificateGSLTRelationalSelectorV1 *selector =
...
bool ppcertificate_gslt_relational_context_v1_declaration_snapshot(
    ...
    static const PPCertificateGSLTRelationalTableRoleV1 roles[] = {
    ...
    PPCertificateGSLTRelationalDeclarationSnapshotV1 snapshot = {0};
    ...
    for (index = 0u; index < sizeof(roles) / sizeof(roles[0]); index++) {
        PPCertificateGSLTRelationalTableRoleV1 role = roles[index];
...
bool ppcertificate_gslt_relational_context_v1_declaration_snapshot_matches(
    ...
    PPCertificateGSLTRelationalDeclarationSnapshotV1 current;
...
ppcertificate_gslt_relational_context_v1_typed_formula(
    ...
    const PPCertificateGSLTSequenceTokenV1 **tokens;
    ...
    *formula_out = (PPCertificateGSLTTokenSequenceV1){
        .tokens = tokens,
...
static PPCertificateGSLTPatternV1 *
ppproof_relational_context_v1_materialize_sequence(
    ...
    while (cursor > 0u) {
        ...
        PPCertificateGSLTPatternV1 *next;
        cursor--;
...
ppcertificate_gslt_relational_context_v1_add_provable_premise(
    ...
    size_t error_buf_size) {
    PPProofRelationalContextImplV1 *impl;
    PPCertificateGSLTPatternV1 *sequence;
    const PPCertificateGSLTPatternV1 *arguments[1];
    PPCertificateGSLTPatternV1 *premise;
...
ppcertificate_gslt_relational_context_v1_fill_premise(
    ...
    if (!context || !(impl = context->implementation) || !premise ||
        reference.kind != PPCERTIFICATE_GSLT_REFERENCE_V1_PREMISE ||
...
static bool ppproof_relational_context_v1_relation_evidence(
    ...
    judgment = impl->evidence_abi->judgments[
        kind == PPPROOF_RELATIONAL_RELATION_V1_DIFFERENT
            ? PPCERTIFICATE_GSLT_SEQUENCE_JUDGMENT_V1_DIFFERENT
            : PPCERTIFICATE_GSLT_SEQUENCE_JUDGMENT_V1_APART];
...
ppcertificate_gslt_relational_context_v1_evidence_sources(
    ...
    }
    {
        ...
        *evidence = (PPCertificateGSLTRelationalEvidenceV1){
            .context = context,
            .active_apartness_table = active_apartness_table,
        };
    }
    *sources_out = (PPCertificateGSLTSequenceEvidenceSourcesV1){
        .context = evidence,
...
static bool ppproof_relational_declaration_v1_prefix_next(
    ...
    if (!impl || !cursor || !row || !found_out ||
        role >= PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_LEN)
        return false;
...
static int ppproof_relational_declaration_v1_order_compare(
    ...
    const PPCertificateGSLTRelationalOrderedRowV1 *left = left_argument;
    const PPCertificateGSLTRelationalOrderedRowV1 *right = right_argument;
...
static bool ppproof_relational_declaration_v1_reference_valid(
    PPCertificateGSLTReferenceV1 reference) {
    return reference.kind == PPCERTIFICATE_GSLT_REFERENCE_V1_PREMISE ||
           reference.kind == PPCERTIFICATE_GSLT_REFERENCE_V1_NODE;
}
...
void ppcertificate_gslt_relational_declaration_v1_free(
    PPCertificateGSLTRelationalDeclarationV1 *declaration) {
    PPCertificateGSLTRelationalDeclarationStorageV1 *storage;
...
ppcertificate_gslt_relational_declaration_v1_elaborate_with_workspace(
    ...
    size_t error_buf_size) {
    PPProofRelationalContextImplV1 *impl;
    PPCertificateGSLTRelationalDeclarationV1 result;
    PPCertificateGSLTRelationalDeclarationStorageV1 *storage = NULL;
    uint32_t *mandatory = NULL;
    uint32_t mandatory_len = 0u;
    uint32_t mandatory_cap = 0u;
    PPCertificateGSLTRelationalOrderedRowV1 *ordered_rows = NULL;
    ...
    ppcertificate_gslt_relational_declaration_v1_init(&result);
    if (error_buf && error_buf_size != 0u)
        error_buf[0] = '\0';
    ...
    for (index = 0u; index < ordered_row_len; index++) {
        ...
        if (!ppproof_relational_context_v1_find(
                impl, PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_LABEL_KIND,
        ...
        if (kind == impl->selector_values[
            ...
            if (!ppproof_relational_declaration_v1_contains(
                ...
                continue;
            if (!ppproof_relational_context_v1_find(
                    impl, PPCERTIFICATE_GSLT_RELATIONAL_TABLE_V1_FORMULA,
            ...
            {
                PPCertificateGSLTRelationalBindingSchemaV1 binding = {
                    .variable = variable,
                    .typecode = typecode,
            ...
            ordered = (PPCertificateGSLTRelationalOrderedHypothesisV1){
                .kind = PPCERTIFICATE_GSLT_RELATIONAL_HYPOTHESIS_V1_BINDING,
            ...
            if (formula.token_len == 0u)
                goto malformed;
            {
                PPCertificateGSLTRelationalEssentialSchemaV1 premise = {
            ...
            ordered = (PPCertificateGSLTRelationalOrderedHypothesisV1){
                .kind = PPCERTIFICATE_GSLT_RELATIONAL_HYPOTHESIS_V1_ESSENTIAL,
...

    {
        ...
        result.conclusion_template = (PPCertificateGSLTTokenSequenceV1){
    ...
    for (;;) {
        ...
        for (prior = 0u; prior < result.disjoint_len; prior++) {
            const PPCertificateGSLTAssertionDisjointV1 *existing =
        ...
        {
            PPCertificateGSLTAssertionDisjointV1 apartness = {left, right};
            if (reuse_scratch) {
                if (cetta_gslt_reusable_buffer_push_v1(
    ...
    ppcertificate_gslt_relational_declaration_v1_free(declaration);
    *declaration = result;
    return PPCERTIFICATE_GSLT_ARTICLE_V1_OK;
...
PPCertificateGSLTArticleV1Result ppcertificate_gslt_relational_declaration_v1_elaborate(
    ...
    char *error_buf,
    size_t error_buf_size) {
    return ppcertificate_gslt_relational_declaration_v1_elaborate_with_workspace(
...
void ppcertificate_gslt_relational_prepared_assertion_v1_free(
    PPCertificateGSLTRelationalPreparedAssertionV1 *prepared) {
    PPCertificateGSLTRelationalPreparedStorageV1 *storage;
...
ppcertificate_gslt_relational_prepared_assertion_v1_build_with_workspace(
    ...
    char *error_buf,
    size_t error_buf_size) {
    PPCertificateGSLTRelationalPreparedAssertionV1 result;
    PPCertificateGSLTRelationalPreparedStorageV1 *storage;
    uint32_t index;
    bool reuse_scratch = false;

    ppcertificate_gslt_relational_prepared_assertion_v1_init(&result);
    if (error_buf && error_buf_size != 0u)
        error_buf[0] = '\0';
    ...
    if (schema->binding_len != 0u) {
        if (reuse_scratch) {
            PPCertificateGSLTAssertionBindingV1 empty = {0};
    ...
    if (schema->essential_len != 0u) {
        if (reuse_scratch) {
            PPCertificateGSLTAssertionEssentialV1 empty = {0};
    ...
    for (index = 0u; index < actual_len; index++) {
        ...
        const PPCertificateGSLTRelationalActualHypothesisV1 *actual =
            &actuals[index];
        ...
        if (ordered->kind ==
            ...
            const PPCertificateGSLTRelationalBindingSchemaV1 *binding;
    ...
    result.declaration = (PPCertificateGSLTAssertionDeclarationV1){
...
ppcertificate_gslt_relational_prepared_assertion_v1_build(
    ...
    char *error_buf,
    size_t error_buf_size) {
    return ppcertificate_gslt_relational_prepared_assertion_v1_build_with_workspace(
