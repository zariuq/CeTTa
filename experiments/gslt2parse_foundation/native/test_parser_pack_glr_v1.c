...
static void recursive_replay_gate(TestCounts *counts, Arena *arena) {
    ...
    (void)expect(
        counts,
        ppglr_v1_parse(&pack, state, input, sizeof(input), 10000u,
                       128u, 128u, &result, error, sizeof(error)),
        error[0] ? error : "recursive ParserPack GLR run");
...
static void ambiguity_and_duplicate_slot_gate(TestCounts *counts,
    ...
    (void)expect(
        counts,
        ppglr_v1_parse(&pack, state, input, sizeof(input), 1000u,
                       64u, 64u, &result, error, sizeof(error)),
        error[0] ? error : "ambiguous ParserPack GLR run");
...
static void deep_ambiguous_equality_gate(TestCounts *counts, Arena *arena) {
    ...
    if (!input) {
        (void)expect(counts, false, "allocate deep ambiguous GLR input");
        return;
    }
    ...
    (void)expect(
        counts,
        ppglr_v1_parse(&pack, root, input, DEEP_INPUT_LEN,
                       5000000u, 64u, 16u,
                       &result, error, sizeof(error)),
        error[0] ? error : "deep ambiguous ParserPack GLR run");
    ...
    (void)expect(
        ...
        "deep equal GLR alternatives deduplicate without merging inequality");
    (void)expect(
        counts,
        !result.canonical_forest_materialized &&
        result.arena.spare_bytes == 0u &&
        result.arena.live_bytes <= (size_t)DEEP_INPUT_LEN * 1024u,
        "large GLR forest canonical scratch is not retained by the result");
    ppnative_v1_result_free(&result);
    ppabi_v1_pack_free(&pack);
    free(input);
}
...
static void class_and_eof_gate(TestCounts *counts, Arena *arena) {
    ...
    (void)expect(
        ...
        ppglr_v1_parse(&pack, state, input, sizeof(input), 1000u,
                       64u, 64u, &result, error, sizeof(error)),
        error[0] ? error : "Unicode class ParserPack GLR run");
    ...
    (void)expect(
        counts,
        ppglr_v1_parse_scalar_view(
            &pack, state, &view, 1000u, 64u, 64u,
            &result, error, sizeof(error)),
        error[0] ? error : "borrowed ParserPack GLR run");
    (void)expect(
        counts,
        result.accepted && result.forest.source_pass_count == 0u &&
            result.forest.decoded_byte_len == 0u &&
            result_has(&result, "(result (pair (cp 955) eof) nil)"),
        "borrowed ParserPack GLR replays without another decode");
    ppnative_v1_result_free(&result);
    ppabi_v1_pack_free(&pack);
}
...
static void typed_boundaries_gate(TestCounts *counts, Arena *arena) {
    ...
    (void)expect(
        counts,
        ppglr_v1_parse(&pack, state, NULL, 0u, 100u, 32u, 32u,
                       &result, error, sizeof(error)) &&
...

    {
        ...
        (void)expect(
            counts,
            !ppglr_v1_parse(&pack, closed_state, invalid_utf8,
...
static void prepared_reuse_gate(TestCounts *counts, Arena *arena) {
    ...
    (void)expect(
        ...
        error[0] ? error : "construct cold ParserPack GLR references");
    error[0] = '\0';
    (void)expect(
        counts,
        ppnative_v1_prepare(
    ...
    (void)expect(
        ...
        error[0] ? error : "reuse prepared ParserPack GLR tables");
    (void)expect(
        counts,
        results_equal(&cold_one, &warm_one) &&
        results_equal(&cold_two, &warm_two) &&
        ppnative_v1_prepared_table_build_count(&prepared) == 1u,
        "prepared ParserPack GLR results equal cold results exactly");
    error[0] = '\0';
    (void)expect(
        counts,
        !ppglr_v1_prepared_parse(
        ...
        "unprepared ParserPack GLR fails closed");
    ...
    (void)expect(
        ...
        "changed ParserPack digest invalidates prepared GLR tables");
    ...
    (void)expect(
        ...
        !ppglr_v1_prepared_parse(
            ...
            10000u, 128u, 128u, &warm_one, error, sizeof(error)) &&
        strstr(error, "not ready") != NULL,
        "freed ParserPack GLR owner fails closed");
    ppabi_v1_pack_free(&pack);
...
int main(void) {
    ...
    printf("(ParserPackGLRV1NativeSummary %u %u)\n",
