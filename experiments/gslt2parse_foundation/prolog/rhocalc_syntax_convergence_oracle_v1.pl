:- module(rhocalc_syntax_convergence_oracle_v1, []).
...
main_ :-
    ...
    cases(Cases),
    format('rhocalc-syntax-convergence-oracle-v1~n', []),
...
compile_pack(PresentationRoot, Syntax, Source, Pack, Start) :-
    pack_case(Syntax, RelativePaths, StartName),
    ...
    require(Outcome = completed(parser_pack_compilation(
            ...
            compiler_completed(Syntax)),
    Start = list([sym('pp-def'), sym(StartName)]).

write_case(Syntax, Label, Text, ExpectedDecision, Source, Pack, Start) :-
...
require(Condition, Label) :-
    ( call(Condition) -> true
    ; throw(error(gate_failed(Label), rhocalc_syntax_convergence_oracle_v1))
    ).
