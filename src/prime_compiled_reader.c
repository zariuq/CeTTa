...
typedef struct {
    ParserHostProjectionV1 *host;
    AtomId shared_anonymous;
} PrimeProjectionV1;
...
static bool prime_projection_begin_form(void *context) {
    ...
    if (!projection ||
        !parser_host_projection_v1_begin_form(projection->host)) {
        return false;
    }
    projection->shared_anonymous = CETTA_ATOM_ID_NONE;
    return true;
}
...
static AtomId prime_projection_word_bytes(
    ...
    return parser_host_projection_v1_word_bytes(
        projection ? projection->host : NULL, bytes, byte_len);
}
...
static AtomId prime_projection_anonymous_variable(void *context) {
    ...
    if (parser_bare_dollar_mode() == PARSER_BARE_DOLLAR_SYMBOL) {
        static const uint8_t bare_dollar[] = {'$'};
        return parser_host_projection_v1_word_bytes(
            projection->host, bare_dollar, sizeof(bare_dollar));
    }
    if (parser_bare_dollar_mode() == PARSER_BARE_DOLLAR_SHARED_VARIABLE &&
        projection->shared_anonymous != CETTA_ATOM_ID_NONE) {
        return projection->shared_anonymous;
    }
    anonymous = parser_host_projection_v1_anonymous_variable(
        projection->host);
    if (parser_bare_dollar_mode() == PARSER_BARE_DOLLAR_SHARED_VARIABLE)
        projection->shared_anonymous = anonymous;
    return anonymous;
}
...
int prime_compiled_reader_v1_parse_bytes_ids(
    ...
    size_t error_buf_size) {
    PrimeProjectionV1 projection_context = {
        .host = NULL,
        .shared_anonymous = CETTA_ATOM_ID_NONE,
    };
    ...
    projection_context.host = parser_host_projection_v1_new(universe);
    if (!projection_context.host) {
        prime_compiled_reader_v1_set_error(
            error_buf, error_buf_size,
    ...
    result = prime_reader_direct_v1_parse_bytes_ids(
        ...
        error_buf, error_buf_size);
    parser_host_projection_v1_free(projection_context.host);
    if (result < 0)
        return -1;
    if (receipt) {
