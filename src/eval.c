...
#define EVAL_PAYLOAD_SPACE_REDIRECT_CAP 64
typedef struct {
    Space *orig[EVAL_PAYLOAD_SPACE_REDIRECT_CAP];
    Space *redirect[EVAL_PAYLOAD_SPACE_REDIRECT_CAP];
    CettaCount len;
} EvalPayloadSpaceRedirectMap;

#define EVAL_PAYLOAD_STATE_REDIRECT_CAP 64
typedef struct {
    StateCell *orig[EVAL_PAYLOAD_STATE_REDIRECT_CAP];
    StateCell *redirect[EVAL_PAYLOAD_STATE_REDIRECT_CAP];
    CettaCount len;
...
bool eval_payload_note_space_redirect(Space *orig, Space *redirect) {
    ...
    if (g_eval_payload_space_redirects.len >= EVAL_PAYLOAD_SPACE_REDIRECT_CAP)
        return false;
...
bool eval_payload_note_state_redirect(StateCell *orig, StateCell *redirect) {
    ...
    if (g_eval_payload_state_redirects.len >= EVAL_PAYLOAD_STATE_REDIRECT_CAP)
        return false;
...
static void metta_eval_one_step(Space *s, Arena *a, Atom *type, Atom *atom,
...

    {
        ...
        if (metta_eval_one_step_expr_congruence(s, a, atom, rs))
            return;
    }
