...
static EqGroup *eq_group_find_or_create(EqGroupSet *gs, SymbolId head_id,
                                        CettaExprLen arity) {
...
static EqGroup *eq_group_lookup(EqGroupSet *gs, SymbolId head_id,
                                CettaExprLen arity) {
    for (size_t i = 0; i < gs->len; i++)
        if (gs->groups[i].head_id == head_id && gs->groups[i].arity == arity)
            return &gs->groups[i];
...
static bool compile_call_args_are_stable(Atom *call_atom, EqGroupSet *groups) {
    ...
    for (CettaExprIndex i = 1; i < call_atom->expr.len; i++) {
...
static bool group_has_known_strict_types(Space *s, Arena *a, EqGroup *g) {
    ...
    for (CettaExprIndex i = 0; i < g->arity; i++) {
...
static void analyze_equations(Space *s, Arena *a, EqGroupSet *out) {
    ...
    for (CettaIndex i = 0; i < logical_len; i++) {
        ...
        SymbolId head_id = SYMBOL_ID_NONE;
        CettaExprLen arity = 0;
...
static void emit_compiled_symbol_name(FILE *out, const char *head,
                                      CettaExprLen arity) {
    fprintf(out, "cetta_");
...
static void capture_init(CaptureTable *ct) {
    ct->caps = NULL;
    ct->len = 0;
    ct->cap = 0;
}

static void capture_free(CaptureTable *ct) {
    free(ct->caps);
    ct->caps = NULL;
    ct->len = 0;
    ct->cap = 0;
...
static void capture_reserve(CaptureTable *ct, size_t needed) {
    if (needed <= ct->cap)
        return;
    size_t next_cap = ct->cap ? ct->cap * 2u : 8u;
    while (next_cap < needed)
        next_cap *= 2u;
    ct->caps = ct->caps
        ? cetta_realloc(ct->caps, sizeof(VarCapture) * next_cap)
        : cetta_malloc(sizeof(VarCapture) * next_cap);
    ct->cap = next_cap;
}
...
static void emit_pattern(FILE *out, const char *atom_reg, Atom *pattern,
    ...
    if (pattern->kind == ATOM_GROUNDED) {
        ...
        case GV_BIGINT:
        {
            ...
            fprintf(out, "  %%chk%d = call i1 @cetta_atom_is_bigint(%%Atom* %s, i8* "
                    "getelementptr([%zu x i8], [%zu x i8]* @str_",
                    l, atom_reg, strlen(text)+1, strlen(text)+1);
            emit_mangled(out, text);
            fprintf(out, ", i32 0, i32 0))\n");
            break;
        }
        case GV_RATIONAL:
        {
            const char *text = atom_rational_cstr(pattern);
            fprintf(out, "  %%chk%d = call i1 @cetta_atom_is_rational(%%Atom* %s, i8* "
                    "getelementptr([%zu x i8], [%zu x i8]* @str_",
                    l, atom_reg, strlen(text)+1, strlen(text)+1);
            emit_mangled(out, text);
            fprintf(out, ", i32 0, i32 0))\n");
    ...
    if (pattern->kind == ATOM_EXPR) {
        ...
        fprintf(out, "exok%d:\n", l);
        fprintf(out, "  %%elen%d = call i64 @cetta_expr_len(%%Atom* %s)\n", l, atom_reg);
        fprintf(out, "  %%elenok%d = icmp eq i64 %%elen%d, %" PRIu64 "\n",
                l, l, (uint64_t)pattern->expr.len);
        fprintf(out, "  br i1 %%elenok%d, label %%earok%d, label %%fail%d\n", l, l, fail_label);
        fprintf(out, "earok%d:\n", l);
        for (CettaExprIndex i = 0; i < pattern->expr.len; i++) {
            int el = g_tmp++;
            fprintf(out, "  %%el%d = call %%Atom* @cetta_expr_elem(%%Atom* %s, i64 %" PRIu64 ")\n",
                    el, atom_reg, (uint64_t)i);
            char elem_reg[16];
...
static const char *emit_direct_call(FILE *out, EqGroup *g, Atom *call_atom,
    ...
    for (CettaExprIndex i = 0; i < nargs; i++) free((void *)arg_regs[i]);
    free(arg_regs);
    return reg;
}
...
static const char *emit_rhs(FILE *out, Atom *rhs, CaptureTable *captures,
    ...
    if (rhs->kind == ATOM_GROUNDED) {
        ...
        case GV_BIGINT:
        {
            ...
            snprintf(reg, sizeof(reg), "%%bigint%d", t);
            fprintf(out, "  %s = call %%Atom* @cetta_atom_bigint(%%Arena* %%arena, i8* "
                    "getelementptr([%zu x i8], [%zu x i8]* @str_",
                    reg, strlen(text)+1, strlen(text)+1);
            emit_mangled(out, text);
            fprintf(out, ", i32 0, i32 0))\n");
            return reg;
        }
        case GV_RATIONAL:
        {
            const char *text = atom_rational_cstr(rhs);
            snprintf(reg, sizeof(reg), "%%rational%d", t);
            fprintf(out, "  %s = call %%Atom* @cetta_atom_rational(%%Arena* %%arena, i8* "
                    "getelementptr([%zu x i8], [%zu x i8]* @str_",
                    reg, strlen(text)+1, strlen(text)+1);
            emit_mangled(out, text);
            fprintf(out, ", i32 0, i32 0))\n");
    ...
    if (rhs->kind == ATOM_EXPR) {
        if (rhs->expr.len == 0) {
            ...
            fprintf(out, "  %s = call %%Atom* @cetta_atom_expr(%%Arena* %%arena, %%Atom** null, i64 0)\n", reg);
            return reg;
        }
        ...
        fprintf(out, "  %s = alloca %%Atom*, i64 %" PRIu64 "\n", reg,
                (uint64_t)rhs->expr.len);
        for (CettaExprIndex i = 0; i < rhs->expr.len; i++) {
            int gi = g_tmp++;
            fprintf(out, "  %%gep%d = getelementptr %%Atom*, %%Atom** %s, i64 %" PRIu64 "\n",
                    gi, reg, (uint64_t)i);
        ...
        fprintf(out, "  %s = call %%Atom* @cetta_atom_expr(%%Arena* %%arena, %%Atom** %%arr%d, i64 %" PRIu64 ")\n",
                reg, t, (uint64_t)rhs->expr.len);
        return reg;
    }
    return "null";
}
...
void compile_space_to_llvm(Space *s, Arena *a, FILE *out) {
    ...
    fprintf(out, "; %zu equation groups, %zu strings\n\n", gs.len, nsyms);
    ...
    fprintf(out, "declare i1 @cetta_atom_is_bigint(%%Atom*, i8*)\n");
    ...
    for (size_t gi = 0; gi < gs.len; gi++) {
        ...
        fprintf(out, "; === %s (arity %" PRIu64 ", %zu equations) ===\n",
                head_name, (uint64_t)g->arity, g->len);
        fprintf(out, "define void @");
        emit_compiled_symbol_name(out, head_name, g->arity);
        fprintf(out, "(%%Space* %%space, %%Arena* %%arena");
        for (CettaExprIndex ai = 0; ai < g->arity; ai++)
            fprintf(out, ", %%Atom* %%arg%" PRIu64, (uint64_t)ai);
        fprintf(out, ", i32 %%fuel, %%ResultSet* %%rs) {\nentry:\n");

        for (size_t ei = 0; ei < g->len; ei++) {
            ...
            if (g->lhs[ei]->kind == ATOM_EXPR) {
                for (CettaExprIndex ai = 1;
                     ai < g->lhs[ei]->expr.len && ai <= g->arity;
                     ai++) {
                    char areg[32];
                    snprintf(areg, sizeof(areg), "%%arg%" PRIu64,
                             (uint64_t)(ai - 1));
            ...
            fprintf(out, "  br label %%next%zu\n", ei);
            fprintf(out, "fail%d:\n  br label %%next%zu\n", fail_lbl, ei);
            fprintf(out, "next%zu:\n", ei);
            capture_free(&captures);
        }
